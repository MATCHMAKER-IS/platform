...
Claude Code / Claude Desktop などの AI アシスタントから、社内基盤（`@platform/*` 103 パッケージ）を**検索できる**ようにする MCP サーバです。

## 何のためにあるか

このリポジトリの根本方針は「**基盤を再利用し、車輪の再発明をしない**」ことです。しかし 103 パッケージ・3,000 以上の公開 API があると、人も AI も「既にあるのか」を把握しきれません。
