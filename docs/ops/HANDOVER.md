...
| 作法の強制 | **36 種類の検査**が `preflight` で自動確認（実行 39 項目・約 15 秒） |
...
| オブジェクトのスプレッド順 | `{ 既定値, ...options }` と書くと、`{ key: undefined }` の指定で**既定値が消える**。`options?.x` をそのまま渡すと必ず踏む。2026-07 に `@platform/context`・`@platform/status-page`(7 関数)・`sheet-grid` の z-index で**同じ誤りが 3 系統**見つかった |
...
- `docs/ops/CHECKS.md` — 36 種類の検査が何を見ているか
