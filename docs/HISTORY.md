...
> ## ⚠️ これは「何を作ったか」の記録で、手引きではありません
>
> **167 節が時系列で並んでいます。** 2026-07-13 が最後の更新で、
> それ以降の変更は反映されていません。**数値もアプリ名も古い**ものが混ざります
> （例: **`equipment-app` は存在しません**——`internal-app` に統合されました）。
>
> **今の状態を知りたいときは、こちらを見てください:**
>
> | 知りたいこと | 見る場所 |
> |---|---|
> | 基盤に何があるか | `docs/ai/module-list.md`（**自動生成**・119 件） |
> | アプリに何があるか | `docs/APPS_AND_DEMOS.md` |
> | 検査は何を守るか | `docs/ops/CHECKS.md`（71 種類） |
> | **なぜこうなっているか** | `docs/ops/HANDOVER.md`（**判断の記録**） |
> | 設計の分かれ道 | `docs/adr/`（22 件） |
>
> **この資料が役に立つのは**、「**この機能はいつ・なぜ入ったのか**」を
> 遡りたいときです。個々の数値は信用せず、**流れだけ**参考にしてください。
...
- 検査: `pnpm smoke`(ロジック回帰) / `check-deps`(循環・層破り) / `api-surface`(公開API差分) / `check-schema`(schema軽量lint) / `check-env-example`(env文書整合) — すべて緑。一括: **`pnpm verify:offline`**(preflight)。セットアップは `bash scripts/setup.sh` / devcontainer(docs/onboarding/01-setup.md)
...
### CI 初回実走の準備
- ci.yml を実走可能な形に修正: pnpm バージョンを packageManager に一本化(9.15.0/10.9.7 不一致解消)、lockfile 未コミットのため install を暫定通常化(TODO 付き)、**Typecheck 前に prisma generate**、**Build にダミー環境変数**(env.ts の fail-fast 対策)、e2e ジョブは整備まで continue-on-error。手順と詰まりどころは `docs/ops/GITHUB_ACTIONS.md`。

### パッケージ物理再配置の段階計画
- 設計判断: **npm 名 @platform/* は不変・パスのみ移動**(= import 無変更)。カテゴリ定義を `tools/package-categories.mjs` に一本化(module-list 生成と共用)。`tools/migrate-packages.mjs`(dry-run 専用)が移動コマンドと**パス直書きの影響箇所を実測**(パイロット=外部SaaS 7個で smoke.mjs の41箇所のみ)。Phase 0(CI緑)→ 1(ツールのパス解決抽象化)→ 2(パイロット)→ 3(依存の少ない順に残り)→ 4(グロブ整理)の手順を `docs/ops/PACKAGE_CONSOLIDATION.md` に明文化。実移行は CI 緑化後。
...
### E2E 整備
- playwright.config.ts の webServer を**配列化**(showcase 3001 / crud-template 3002 / equipment-app 3003)。`e2e/crud-template.spec.ts`(登録→編集→無効化→無効表示)と `e2e/internal-equipment.spec.ts`(ログイン→登録→貸出→返却)を追加。**未実走**(オフラインのため)・CI 緑化後に確認の注記付き。
...
- **docs/onboarding/01-setup.md**: クイックスタート / 前提 / スクリプトの中身 / ポート一覧(3000〜3004, 5432, 1025/8025, 7700, 6379)/ アプリ別 DB の理由 / Prisma 運用(push と migrate の使い分け・--schema 指定)/ つまずき FAQ / 手動手順 / Windows は WSL2 推奨。README 冒頭にもクイックスタートを追加(パッケージ数 81→97 の古い記述も修正)。
...
- **docs/ops/GITHUB_ACTIONS.md を結果記入欄つきチェックリスト(10手順+つまずき分岐)へ刷新**。
...
- ショートカット10種: `db:up/down/psql`、`dev:internal/site/crud/equipment/demos`、`mcp`。../onboarding/01-setup.md に一覧表。
...
- 壁打ち最優先の「Platform Portal」最小版。`catalog.ts` が**リクエスト時にリポジトリ成果物**(api-surface/module-list/README/platform-report/adr)を読み、`/api/catalog` が返す。3タブ(パッケージ検索=名前/説明/export+カテゴリ絞り / ヘルス / ADR)。port 3005。120パッケージを AI も人も探せる入口。
...
- 各パッケージ src/index.ts から export と JSDoc 要約を抽出(**120 パッケージ・422 エントリ・要約付き99.8%**)。TypeDoc の完全版は重い(要 install)ため軽量自作。
...
### ER図生成(tools/gen-erd.mjs → apps/<アプリ名>/docs/erd.md*.md・Portal統合)
...
### 画面・API 一覧生成(tools/gen-app-map.mjs → apps/<アプリ名>/docs/appmap.md*.md・Portal統合)
...
- **tools/check-win-setup.mjs**(preflight 組込): pwsh 無し環境でも回る静的検査(括弧均衡・必須要素・sh 版との手順対応 17項目)。../onboarding/01-setup.md に Windows 手順を追記。
...
### Windows セットアップ(scripts/setup.ps1 + setup.bat)
- setup.sh と**完全同等**の PowerShell 版 + バッチラッパー。前提確認(-Check)・Docker省略(-SkipDocker)・スキーマ省略(-SkipDb)対応。pwsh/powershell 自動判定。前提確認→.env準備→Docker起動→アプリ別DB作成→pnpm install→prisma generate/push×3→スモーク検証まで。../onboarding/01-setup.md に Windows 手順を記載。
...
### CI 実走準備(①)
- install 不要の全ゲート(preflight 11項目・スモーク911・check-generated・全8ワークフロー/amplify/compose の YAML・Windows 静的検証)をローカルで緑確認。残るは install/build/docker/e2e の実走のみ。実ログが出れば GITHUB_ACTIONS.md のチェックリストで即対応できる状態。
...
### リネーム + 機能カタログの配置変更
- リポジトリ名を `platform-monorepo` → `platform` に統一(package.json / devcontainer / MCP 設定例 / ../onboarding/01-setup.md)。
- 機能カタログ `docs/HISTORY.md` を `docs/platform/` から **リポジトリ直下**(apps/packages と同階層)へ移動。参照(docs/ai/architecture.md)も更新。
...
- 基盤 `docs/site/index.html`: 120パッケージ + 465公開API を検索で絞り込み + 依存グラフ（Mermaid）。
...
- **目的**: 120 パッケージ・3,000+ API があると、人も AI も「既にあるか」を把握しきれず**車輪の再発明**が起きる。AI が実装前に自分で基盤を検索できるようにする。
...
- **点検で発見**: CLAUDE.md と architecture.md が「120 パッケージ」と古い数値のままだった(実際は 103)。**AI はこれを読んで前提にするため、古い数値は誤った判断を生む**。
...
- **現状**: 120 パッケージすべて `0.1.0` / `private: true`。アプリは `workspace:*` でローカルの packages/ を直接参照するため、**基盤を編集した瞬間からアプリは新しいコードを使う**(install も再ビルドも不要)。
...
- **発見**: demos の README(当時。現在 demos は廃止)は「デモは `pnpm dev` だけで動く」と書いていたが、**実際は 25 個中 24 個が dev スクリプトを持たず起動しない**(コンポーネント集)。
...
### 【新規】ゼロからの導入ガイド(docs/onboarding/01-setup.md ＋ _2.md)
...
- README / ../onboarding/01-setup.md から導線。../onboarding/01-setup.md は「ツールは入っている前提の要点整理」として棲み分け。
...
- 実際に **docs/HISTORY.md のパス誤り(public-site の preview.ts で `src` が抜けていた)を発見・修正**。例示(`my-app`)や MCP のメソッド名(`tools/list`)は誤検出しないよう除外。
...
### 【発見】../onboarding/01-setup.md のポート記述が古かった
- **問題**: ポート統一の修正(前セッション)が ../onboarding/01-setup.md に反映されておらず、**`public-site: pnpm --filter public-site dev -- -p 3004(既定 3000 と衝突するため)` という既に解消済みの記述**が残り、platform-portal(3005)も欠落していた。
- **対応**: 実態に合わせて修正。さらに **check-ports の検査対象に ../onboarding/01-setup.md のポート表を追加**し、記述のドリフトを機械検出できるように(エラーメッセージにどのファイルがズレているかも表示)。

### ../onboarding/01-setup.md と GETTING_STARTED の重複整理
- **役割分担**: **GETTING_STARTED = 初心者向けの流れ**(ツール導入から順に) / **../onboarding/01-setup.md = 経験者向けリファレンス**(setup スクリプトの中身・Prisma の運用・devcontainer など手順の裏側)。../onboarding/01-setup.md 冒頭に対応表を置き、初心者は GETTING_STARTED へ誘導。
- **重複の解消**: 一般的なつまずき(Docker 未起動・pnpm バージョン等)は GETTING_STARTED_2 に集約し、../onboarding/01-setup.md には固有のもの(Prisma engines の DL 失敗・devcontainer の DB ホスト名・本番の秘密値強度)だけを残す。Windows の節も「ツールの入れ方は GETTING_STARTED、ここは setup.ps1 の仕様」と分離。「次に読むもの」も最新の資料(NEW_APP / patterns / docs 地図)に更新。
...
- 前回 ../onboarding/01-setup.md の重複を**手作業で**見つけたため機械化。同一見出し・同一の表(3行以上)が複数ファイルにある状態を検出し、「片方だけ更新されて食い違う」事故を防ぐ。
...
### 【新規】業務パターンの負荷シナリオ(packages/loadtest)
...
### 構想の評価と絞り込み
提案された 14 機能のうち、**既存と重複するもの**(Config Viewer → `/admin/env`、Performance → `/admin/ops`、AI Review → preflight/advisor の 15 検査、Session Viewer → `/api/auth/me`)と、**技術的に非現実的なもの**(Package Monitor: 実行時に「どのパッケージを使ったか」を知るには 120 パッケージ全てに計装が必要。基盤を汚す)を除外し、**核心の 4 つ**に絞って実装した。
...
- **基盤側に計装を仕込まない**設計。120 パッケージに手を入れず、呼び出し側が記録したいものだけ記録する。
...
- **発見**: `e2e.yml` が **`prisma migrate deploy` を実行**していたが、**マイグレーションファイルが 1 つも無い**(開発は `db push` 運用)。**何も適用されず E2E が失敗する**状態だった。../onboarding/01-setup.md も「チーム開発・本番はマイグレーション履歴を推奨」と書いており、実態と食い違っていた。
...
- CI を `db push` に修正、../onboarding/01-setup.md も整合させた。**「本番運用を開始する」が見直しの最重要条件**と明記。
...
## TSDoc: 小規模 11 パッケージを完備 — **69%・完備 120 パッケージ**
...
**完備は 120 パッケージ**。残るのは `mobile`(36)/ `booking`(32)/ `mail`(30)/ `invoice`(22)/ `notify`(21)/ `line`(18)。
...
**完備は 120 パッケージ**。残るのは `mail`(30)/ `invoice`(22)/ `notify`(21)/ `line`(18)/ `cron`(7)。
...
**完備は 120 パッケージ**。残るのは `db`(26)/ `google`(8)/ `sms`(7)/ `cron`(7)/ `faker`(7)/ `csv`(5)など。
...
# 🎉 TSDoc: 全 1,691 関数・120 パッケージが完備(100%)
...
**デプロイ対象を 1 つに絞る**ため、`apps/showcase` を統合サイトに拡張した。
...
│   └── /apps/portal      基盤ポータル(120 パッケージを検索)
...
- `appRoot: apps/showcase`(モノレポとして設定)
...
### 最終形
```
apps/showcase(:3001・45画面・DB依存 0)
...
- **`loadDemos` を nav.ts から読む方式に**変更(以前は `demos/*` を走査)。
  **サイトの表示と、AI の検索結果が食い違わない**のが利点。81 デモを返す。
- `check-doc-numbers` の「コンポーネント型」検査を「統合デモサイト数」に(概念が無くなったため)
- demos の README(当時)を全面書き換え(1 サイトに集約した理由・Amplify 手順・実物は apps/ にあること)
...
**修正**: `appRoot: apps/showcase` に。`corepack prepare pnpm@9.15.0` も明示(Amplify の
既定イメージに pnpm は入っていない)。

### 【2】amplify.yml が 2 つあった
`apps/showcase/` にも作っていたが、**Amplify はリポジトリ直下しか読まない**。
...
- **モノレポのルートディレクトリに `apps/showcase` を設定する**(これをしないと失敗)
...
## 【発見】`pnpm dev:showcase` が動かなかった

lock を作る前の確認で見つけた。ルートの `package.json` に:

```json
"dev:showcase": "pnpm --filter @apps/showcase dev"
```

とあったが、**実際のパッケージ名は `showcase`**。`@apps/showcase` は存在しないので、
**このコマンドは失敗する**(README に書いてあるのに動かない状態だった)。

**修正**: `pnpm --filter showcase dev` に。
...
```yaml
- cd ../.. && pnpm --filter showcase build   # ❌ 私が書いたもの
```

これだと **Next.js がモノレポのルートで動く**。Turbopack が `./apps/showcase/...` を
...
build:
  commands:
    - pnpm build   # ✅ appRoot(apps/showcase)のまま実行
```
...
**修正**: 全 120 パッケージの tsconfig に `"exclude": ["**/*.test.ts", "**/*.test.tsx"]` を追加。
...
- **install は成功していた**(`Scope: all 114 workspace projects`)。
  ログに `../..` と出ていたので、**Amplify は最初から appRoot(apps/showcase)で実行している**。
- つまり preBuild の `cd ../..` は正しく動き、その cd が build に残るのが問題だった。

### 最終的な対策
**build で cd と next build を 1 コマンドにまとめる**:
```yaml
- pwd && cd apps/showcase && pwd && pnpm exec next build
```
...
2. build で `pnpm build` → cd が残り、ルートの turbo が**全 120 パッケージ**をビルド
...
- **cd も成功**(`pwd` → `/codebuild/.../src/platform/apps/showcase`)
- **Next.js 16.2.0 (Turbopack) が起動**
- なのに **Module not found が 103 件**

### 決定的だったのはエラーのパス表示
```
./apps/showcase/src/app/examples/accounting-sync/page.tsx:1:1
```
**cwd は `apps/showcase` なのに、Turbopack は `./apps/showcase/...` と表示**している。
...
> **【撤回】** このオプションは存在せず、フォールバックもできていなかった。7 回目を参照。
...
We couldn't find the Next.js package (next/package.json) from the project directory:
  /codebuild/.../apps/showcase/src/app
To fix this, set turbopack.root in your Next.js config
```

### 何を間違えたか
`turbopack.root: __dirname`(= apps/showcase)にした。root の指定自体は効いた
(パス表示が `./apps/showcase/...` → `./src/app` に変わった)。
...
## Amplify ビルド 6 回目 — Turbopack を諦め、webpack にした 【この判断は誤り・撤回済み】

> **【撤回】** 以下の「webpack にする」判断は**前提から間違っていた**。
> `--no-turbopack` は Next.js に存在しないオプションで、書いても黙って無視される
> (opt-out は `--webpack`)。つまり**この間ずっと Turbopack でビルドされていた**。
> 真因は次節の `dist` で、Turbopack は最初から無実だった。
> 現在は `"build": "next build"`(Turbopack)で通っている。
> **記録として残すが、ここに書かれた対処を真似しないこと。**
...
### 判断: webpack を使う 【誤り】
```json
"build": "next build --no-turbopack"   // ❌ 存在しないオプション。無視される
```

**当時の理由**(いずれも誤った観測にもとづく):
...
`pnpm --filter showcase build` が通ることを確認してから**。

経緯は `next.config.mjs` のコメントに残した(次に触る人が同じ 3 通りを試さないように)。

### amplify.yml も修正
`pnpm exec next build` だと **package.json のスクリプトを経由しない**ので、
`pnpm run build` に変えた。

> 当時は「これで `--no-turbopack` が効く」と考えたが、そのオプション自体が無効だった。
> ただし **`pnpm run build` に揃えたこと自体は正しい**ので、この変更は残している。
...
### 今回見つけた本当のバグ
1. **`main` が存在しない `dist` を指していた**(120 パッケージ)→ src に統一
