...
export function decayRate(usefulLifeYears: number): number {
   ...
   *
   * **`internal-app` に繋いであります(2026-08)。**
   * `business-metrics` が `business.stale_assets` として数え、
   * **固定資産税の申告前に確認**できます。
   *
   * **廃棄した資産を申告し続けると、払わなくてよい税を払い続けます。**
   *
   * **画面に出すより、申告前のチェックリストに入れる方が確実**です
   * ——毎日見る画面に出しても、年 1 回の作業では思い出しません。
   *
   * **型が違う点に注意**——アプリの `FixedAsset` は `cost`、
   * こちらは `acquisitionCost` という名前です(繋ぐ側で変換しています)。
   */
