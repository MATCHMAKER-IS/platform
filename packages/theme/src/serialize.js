...
/** ThemeTokens の必須キー。 */
const TOKEN_KEYS = ["bg", "fg", "muted", "surface", "border", "primary", "primaryFg", "accent", "success", "warning", "danger"];
...
export function validateTheme(value) {
    ...
    const t = value;
    if (typeof t.id !== "string" || !isValidThemeId(t.id))
        issues.push({ path: "id", message: "id が不正です（英数字・ハイフンのみ、1〜40 文字）" });
    if (typeof t.name !== "string" || !t.name.trim())
        issues.push({ path: "name", message: "name が空です" });
    if (t.description !== undefined && typeof t.description !== "string")
        issues.push({ path: "description", message: "description は文字列です" });
    const shape = t.shape;
    if (!shape || typeof shape !== "object") {
        issues.push({ path: "shape", message: "shape がありません" });
    }
    else {
        if (typeof shape.fontFamily !== "string" || !shape.fontFamily.trim())
            issues.push({ path: "shape.fontFamily", message: "fontFamily が空です" });
        if (typeof shape.radius !== "number" || shape.radius < 0 || shape.radius > 100)
            issues.push({ path: "shape.radius", message: "radius は 0〜100 の数値です" });
        if (typeof shape.spacing !== "number" || shape.spacing < 0 || shape.spacing > 100)
            issues.push({ path: "shape.spacing", message: "spacing は 0〜100 の数値です" });
        if (![0, 1, 2, 3].includes(shape.elevation))
            issues.push({ path: "shape.elevation", message: "elevation は 0〜3 です" });
    }
    const modes = t.modes;
        ...
        for (const mode of ["light", "dark"]) {
            ...
            for (const key of TOKEN_KEYS) {
                if (!isColor(tokens[key]))
                    issues.push({ path: `modes.${mode}.${key}`, message: `${key} が色として不正です` });
            }
        }
    }
    return issues;
 ...
 * @returns 整形済みの JSON 文字列
 */
export function themesToJson(themes) {
 ...
 */
export function themesFromJson(json) {
    let parsed;
    try {
    ...
    if (Array.isArray(parsed))
        return parsed.map(parseTheme);
    if (typeof parsed === "object" && parsed !== null && Array.isArray(parsed.themes)) {
        return (parsed.themes).map(parseTheme);
    }
