 ...
 * S3 互換 Adapter を作る。
 *
 * **キーの扱いがローカル実装と違う。** S3 のキーは単なる文字列で、
 * スラッシュは階層の見た目でしかない——`"uploads/../secret"` は
 * **そのままのキー**になり、`../` でバケットの外へは出られない。
 * (ローカル実装は実際のパスなので外へ出る。あちらは検証して弾いている)
 *
 * **ただし prefix による分離は破れうる。** `tenant-a/` で顧客を分けている場合、
 * キーに `../` を含めると、**正規化する実装**(SDK のバージョン・CDN・
 * 前段のプロキシ)によっては `tenant-b/` を指すことがある。
 * **利用者の入力をキーにするなら、`../` を含むものは呼び出し側で弾くこと**
 * (2026-08 に明記)。
 *
 * 署名付き URL の既定は 15 分。長くすると、URL が漏れた場合の猶予が伸びる。
 *
 * @param config 接続設定
