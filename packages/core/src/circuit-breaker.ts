 ...
 *
 * **`@platform/observability` にも同名の実装がある。**
 *
 * | | 使うとき |
 * |---|---|
 * | `@platform/observability` | **ふつうはこちら。** 開閉だけを見る 102 行の実装 |
 * | `@platform/core`(ここ) | 半開で通す本数を制御したい / 状態変化を受け取りたい |
 *
 * 統合しないのは、**求めるものが違う**ため。
 * 監視の文脈では「落ちているか」だけ分かればよく、
 * 細かい制御は却って設定を増やす(ADR-0015 の判断基準)。
 *
