 ...
 */
export function buildNotifyCommand(platform, n) {
    const title = n.title;
 ...
 */
export function buildSoundCommand(platform, soundFile) {
    if (platform === "win32") {
        if (soundFile) {
 ...
 * @returns 履歴ストア
 */
export function createMemoryNotifyLog(options = {}) {
    const max = options.max ?? 200;
    const entries = [];
    return {
        append(entry) { entries.push(entry); if (entries.length > max)
            entries.shift(); },
...
export function createOsNotifier(options = {}) {
