 ...
 *
 * **市外局番が 4 桁の地域では区切りを誤る。** 実装は「主要 2 桁(03/06)と
 * それ以外は 3 桁」という一般則の近似で、**4 桁の市外局番(青梅 0428・
 * 富士吉田 0555・佐渡 0259 など多数)は 3 桁として切る**
 * ——`0428-12-3456` が `042-812-3456` になり、**042(八王子・立川)の番号に見える**。
 *
 * 桁数は変わらないので**発信はできる**が、帳票や宛名ラベルに出すと
 * **別の地域だと誤解される**。正確に整形するには**総務省の市外局番一覧**
 * (数千件)が要るので、この関数では持たない。
 *
 * 表示の正確さが要る用途では、**入力されたままの区切りを保存して使うこと**
 * (`normalizePhone` で潰さない)。2026-08 に明記。
 *
 * @param input 電話番号
 ...
 *
 * **残す桁数以下の入力は全部伏せる**(日本の電話番号は最短 10 桁なので、
 * 4 桁以下は不正な値)。`@platform/pii` の `maskPhone` と同じ挙動。
 *
 * @param input 電話番号
...
export function maskPhone(input: string, visible = 4): string {
  ...
  // 日本の電話番号は最短でも 10 桁なので、4 桁以下は**不正な値**。
  // 不正な値をそのまま返すと、**内線番号や打ち間違いが画面やログに出る**。
  // `@platform/pii` の `maskPhone` と同じ挙動に揃えた。
  if (n.length <= visible) return "*".repeat(n.length);
