...
export function BackToTop({ threshold = 300, target, className, label = "ページトップに戻る" }: BackToTopProps) {
  ...
  return (
    <button
      ...
      title={label}
      className={cn(
        "fixed bottom-6 right-6 z-50 flex h-11 w-11 items-center justify-center rounded-full border border-[var(--color-border)] bg-[var(--color-bg)] text-[var(--color-fg)] shadow-lg transition-opacity hover:bg-slate-50",
        className,
      )}
    >
