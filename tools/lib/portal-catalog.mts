...
// **生成時に使う。** 以前はアプリ内で `process.cwd()` を起点にしていたが、
// 実行時にファイルを読む作りは配置に依存して壊れる(Amplify の SSR で
// cwd が想定と違う場所を指す)。ここは生成専用なので import.meta 起点で固定する。
...
function collectUsedPackages(): Set<string> {
  ...
  for (const group of ["apps"]) walk(path.join(ROOT, group), null);
