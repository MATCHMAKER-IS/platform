      ...
      {draft && (
        <div className="mb-4 rounded border border-[color-mix(in_srgb,var(--color-warning)_40%,transparent)] p-3">
          ...
          <table className="w-full text-sm">
            <tbody>
              {draft.lines.map((l, i) => (<tr key={i} className="border-b border-[var(--color-border)]"><td className="px-2 py-1">{l.description}</td><td className="px-2 py-1 text-right">{l.quantity}</td></tr>))}
            </tbody>
          </table>
          <p className="mt-2 text-xs text-[var(--color-muted)]">単価は仮（0円）です。実際の発注では単価・仕入先を補記してください。</p>
        </div>
      )}
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.product.sku} className="border-b border-[var(--color-border)]">
              ...
              <td className="px-2 py-2 text-right text-[var(--color-muted)]">{r.summary.totalOut}</td>
              <td className="px-2 py-2">{r.needsReorder ? <span className="rounded bg-[color-mix(in_srgb,var(--color-warning)_15%,transparent)] px-2 py-0.5 text-xs text-[var(--color-warning)]">要発注 {r.suggestedOrderQty}</span> : <span className="text-xs text-[var(--color-muted)]">—</span>}</td>
            </tr>
          ))}
          {rows.length === 0 && <tr><td colSpan={6} className="px-2 py-4 text-center text-sm text-[var(--color-muted)]">商品がありません。</td></tr>}
        </tbody>
      </table>
        ...
        <div className="mb-6 rounded border border-[var(--color-border)] p-4">
          ...
          {(detail.expired.length > 0 || detail.expiringSoon.length > 0) && (
            <div className="mb-3 text-xs">
              {detail.expired.length > 0 && <p className="text-[var(--color-danger)]">期限切れ: {detail.expired.map((l) => `${l.lotId}(${l.quantity})`).join("、")}</p>}
              {detail.expiringSoon.length > 0 && <p className="text-[var(--color-warning)]">期限間近: {detail.expiringSoon.map((l) => `${l.lotId}(${l.quantity}・${l.expiry ?? ""})`).join("、")}</p>}
            </div>
          )}
          <table className="w-full text-xs">
            <thead><tr className="border-b border-[var(--color-border)] text-left text-[var(--color-muted)]"><th className="px-2 py-1">日時</th><th className="px-2 py-1">種別</th><th className="px-2 py-1 text-right">数量</th><th className="px-2 py-1">倉庫/ロット</th><th className="px-2 py-1">参照</th></tr></thead>
            <tbody>
              {detail.movements.map((m, i) => (
                <tr key={i} className="border-b border-[var(--color-border)]">
                  <td className="px-2 py-1 text-[var(--color-muted)]">{m.at.slice(0, 16).replace("T", " ")}</td>
                  <td className="px-2 py-1">{MOVE_LABEL[m.type]}</td>
                  <td className="px-2 py-1 text-right">{m.quantity}</td>
                  <td className="px-2 py-1 text-[var(--color-muted)]">{[m.warehouse, m.lotId].filter(Boolean).join(" / ") || "—"}</td>
                  <td className="px-2 py-1 text-[var(--color-muted)]">{m.ref ?? "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <div className="rounded border border-[var(--color-border)] p-4">
            ...
            <div className="flex flex-col gap-2">
              <Select
                value={move.sku} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setMove({ ...move, sku: e.target.value })} className="rounded border border-[var(--color-border)] px-2 py-1 text-sm"
                options={[
              ...
              <div className="flex gap-2">
                <Select
                  value={move.type} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setMove({ ...move, type: e.target.value as typeof move.type })} className="rounded border border-[var(--color-border)] px-2 py-1 text-sm"
                  options={[
                ...
                />
                <Input value={move.quantity} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setMove({ ...move, quantity: e.target.value })} placeholder="数量" inputMode="numeric" className="w-24 rounded border border-[var(--color-border)] px-2 py-1 text-sm" />
                <Input value={move.ref} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setMove({ ...move, ref: e.target.value })} placeholder="参照（発注/出荷番号）" className="flex-1 rounded border border-[var(--color-border)] px-2 py-1 text-sm" />
              </div>
              <div className="flex gap-2">
                <Input value={move.warehouse} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setMove({ ...move, warehouse: e.target.value })} placeholder="倉庫（任意）" className="flex-1 rounded border border-[var(--color-border)] px-2 py-1 text-sm" />
