      ...
      {creating && (
        <div className="mb-6 rounded border border-[var(--color-border)] p-4">
          <div className="mb-3 grid gap-2 md:grid-cols-2">
            <Input value={header.number} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeader({ ...header, number: e.target.value })} placeholder="プラン番号（例 SUB-001）" className="rounded border border-[var(--color-border)] px-2 py-1 text-sm" />
            ...
            <label className="text-xs text-[var(--color-muted)]">周期
              <Select
                value={header.interval} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setHeader({ ...header, interval: e.target.value })} className="mt-0.5 block w-full rounded border border-[var(--color-border)] px-2 py-1 text-sm"
                options={[
            ...
            <label className="text-xs text-[var(--color-muted)]">開始日<Input type="date" value={header.startDate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeader({ ...header, startDate: e.target.value })} className="mt-0.5 block w-full rounded border border-[var(--color-border)] px-2 py-1 text-sm" /></label>
          </div>
