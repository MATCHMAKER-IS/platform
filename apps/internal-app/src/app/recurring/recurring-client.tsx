        ...
        <div className="mb-6 rounded border border-neutral-200 p-4">
          <div className="mb-3 grid gap-2 md:grid-cols-2">
            ...
            <label className="text-xs text-neutral-500">周期
              <select value={header.interval} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setHeader({ ...header, interval: e.target.value })} className="mt-0.5 block w-full rounded border border-neutral-300 px-2 py-1 text-sm">
                <option value="monthly">毎月</option><option value="quarterly">四半期</option><option value="yearly">毎年</option>
              </select>
            </label>
            ...
            <tbody>
              {lines.map((l, i) => (
                <tr key={i}>
                  ...
                  <td className="px-1 py-1"><select value={String(l.taxRate ?? 10)} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setLine(i, { taxRate: Number(e.target.value) as 10 | 8 | 0 })} className="w-full rounded border border-neutral-300 px-1 py-1"><option value="10">10%</option><option value="8">8%</option><option value="0">0%</option></select></td>
