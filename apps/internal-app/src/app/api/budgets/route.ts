...
async function handleGET(req: Request): Promise<Response> {
  ...
  const period = new URL(req.url).searchParams.get("period") ?? formatMonthJst();
...
async function handlePOST(req: Request): Promise<Response> {
  ...
  const body = (await req.json().catch(() => ({}))) as BudgetLine;
  // **金額は整数で受ける。** 小数を通すと Int 列への書き込みで落ちる。
  const badAmount = !Number.isSafeInteger(body.amount) || body.amount <= 0;
  if (!body.department || !body.category || !/^\d{4}-\d{2}$/.test(body.period ?? "") || badAmount) {
    return Response.json({ error: "部門・区分・期間(YYYY-MM)・1 円以上の整数の金額が必要です" }, { status: 400 });
  }
