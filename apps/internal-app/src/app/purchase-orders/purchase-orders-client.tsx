      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        {canWrite && (
          <span className="flex items-center gap-2">
            ...
            <Button onClick={createFromReorder} className="rounded bg-[var(--color-fg)] px-4 py-2 text-sm text-white">発注点割れから起票</Button>
          </span>
        )}
      </div>
          ...
          {orders.map((o) => (
            <React.Fragment key={o.number}>
              <tr className="border-b border-[var(--color-border)]">
                <td className="px-2 py-2 font-mono text-xs"><Button onClick={() => setOpen(open === o.number ? null : o.number)} className="text-[var(--color-primary)] hover:underline">{o.number}</Button>{canWrite && <Button onClick={() => submitApproval(o.number, o.order.totals.total)} className="ml-2 text-xs text-[var(--color-primary)] hover:underline">承認申請</Button>}</td>
                    ...
                    <tbody>
                      {o.lineStatus.map((ls) => (
                        <tr key={ls.lineIndex} className="border-t border-[var(--color-border)]">
                          ...
                          <td className="px-2 py-1 text-right">{canWrite && !ls.complete && o.status !== "cancelled" && <Button onClick={() => receive(o.number, ls.lineIndex, ls.outstanding)} className="text-[var(--color-primary)] hover:underline">入荷</Button>}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
