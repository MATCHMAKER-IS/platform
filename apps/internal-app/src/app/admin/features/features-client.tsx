    ...
    <div className="mx-auto max-w-3xl p-6">
      ...
      <div className="overflow-x-auto rounded border border-[var(--color-border)]">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-[var(--color-border)] text-xs text-[var(--color-muted)]"><th className="px-3 py-2 text-left">機能</th><th className="px-2 py-2">有効</th>{ROLES.map((r) => <th key={r} className="px-2 py-2">{ROLE_LABEL[r]}</th>)}</tr></thead>
          <tbody>
            {catalog.map((f) => {
              const rule = rules[f.key];
              if (!rule) return null;
              return (
                <tr key={f.key} className={`border-b border-[var(--color-border)] ${rule.enabled ? "" : "bg-[var(--color-subtle)] opacity-60"}`}>
                  <td className="px-3 py-2">{f.label}<span className="ml-1 text-xs text-[var(--color-muted)]">{f.href}</span></td>
                  <td className="px-2 py-2 text-center"><Checkbox  checked={rule.enabled} onCheckedChange={() => toggleEnabled(f.key)} /></td>
                  {ROLES.map((role) => (
                    <td key={role} className="px-2 py-2 text-center">
                      {role === "admin" ? <span className="text-xs text-[var(--color-muted)]">常に可</span> : <Checkbox  disabled={!rule.enabled} checked={rule.roles.length === 0 || rule.roles.includes(role)} onCheckedChange={() => toggleRole(f.key, role)} />}
                    </td>
                  ))}
                </tr>
              );
