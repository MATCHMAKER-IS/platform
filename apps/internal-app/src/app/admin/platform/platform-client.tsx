  ...
  return (
    <div className="mx-auto max-w-3xl p-6">
      ...
      <div className="mb-4 flex gap-1 border-b border-[var(--color-border)]">
        {[["secrets", "秘密情報"], ["flags", "フィーチャーフラグ"]].map(([k, l]) => <Button key={k} onClick={() => setTab(k!)} className={`px-3 py-2 text-sm ${tab === k ? "border-b-2 border-[var(--color-fg)] font-medium" : "text-[var(--color-muted)]"}`}>{l}</Button>)}
      </div>
      ...
      {tab === "secrets" && (
        <div>
          <div className="mb-4 rounded border border-[var(--color-border)] p-4">
            <p className="mb-2 text-xs text-[var(--color-muted)]">外部API資格情報やWebhook secretを暗号化して保存します（値は保存後に表示できません）。同名で保存するとローテーションになります。</p>
            <div className="grid grid-cols-3 gap-2">
              <Input value={sName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSName(e.target.value)} placeholder="名前 (例 STRIPE_KEY)" className="rounded border border-[var(--color-border)] px-2 py-1 text-sm" />
              <Input value={sValue} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSValue(e.target.value)} placeholder="値" type="password" className="rounded border border-[var(--color-border)] px-2 py-1 text-sm" />
              <Button onClick={saveSecret} className="rounded bg-[var(--color-fg)] px-3 py-1 text-sm text-white">保存/ローテーション</Button>
            </div>
          </div>
          <ul className="divide-y divide-neutral-100 rounded border border-[var(--color-border)]">
            {secrets.map((s) => <li key={s.name} className="flex items-center justify-between px-3 py-2 text-sm"><code>{s.name}</code><span className="text-xs text-[var(--color-muted)]">更新: {s.updatedAt.slice(0, 10)}</span></li>)}
            {secrets.length === 0 && <li className="px-3 py-6 text-center text-sm text-[var(--color-muted)]">秘密情報はありません。</li>}
          </ul>
        </div>
      )}
      ...
      {tab === "flags" && (
        <div className="rounded border border-[var(--color-border)] p-4">
          <p className="mb-2 text-xs text-[var(--color-muted)]">フラグ定義（JSON）。<code>{'{ "flag": { "enabled": true, "rolloutPercent": 20 } }'}</code> の形式。enabled:false で緊急停止、rolloutPercent で段階的ロールアウト、variants で A/B。</p>
          <Textarea value={flagJson} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setFlagJson(e.target.value)} rows={12} className="block w-full rounded border border-[var(--color-border)] px-2 py-1 font-mono text-xs" />
          <Button onClick={saveFlags} className="mt-2 rounded bg-[var(--color-fg)] px-4 py-2 text-sm text-white">フラグを更新</Button>
        </div>
      )}
    </div>
  );
