...
export function PlatformClient({ fetchImpl }: { fetchImpl?: typeof fetch }) {
  ...
  return (
        <PageShell title="秘密情報・フラグ">
      <div className="mb-4 flex gap-1 border-b border-[var(--color-border)]">
        {[["secrets", "秘密情報"], ["flags", "フィーチャーフラグ"]].map(([k, l]) => <Button key={k} onClick={() => setTab(k!)} variant="tab" data-state={tab === k ? "active" : undefined}>{l}</Button>)}
      </div>
      ...
      {tab === "secrets" && (
       ...
       <Button onClick={saveSecret} className="rounded px-3 py-1 text-sm text-white">保存/ローテーション</Button>
            </div>
          </div>
          <ul className="divide-y divide-neutral-100 rounded border border-[var(--color-border)]">
            {secrets.map((s) => <li key={s.name} className="flex items-center justify-between px-3 py-2 text-sm"><code>{s.name}</code><span className="text-xs text-[var(--color-muted)]">更新: {formatDateJst(new Date(s.updatedAt))}</span></li>)}
      ...
      {tab === "flags" && (
        <div className="rounded border border-[var(--color-border)] p-4">
          ...
          <Textarea aria-label="フィーチャーフラグ(JSON)" value={flagJson} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setFlagJson(e.target.value)} rows={12} className="block w-full rounded border border-[var(--color-border)] px-2 py-1 font-mono text-xs" />
     <Button onClick={saveFlags} className="mt-2 rounded px-4 py-2 text-sm text-white">フラグを更新</Button>
        </div>
      )}
    </PageShell>
  );
