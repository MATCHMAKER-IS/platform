...
export function DbViewerClient({ fetchImpl }: { fetchImpl?: typeof fetch }) {
  ...
  const sidebar: React.CSSProperties = { width: 220, borderRight: "1px solid var(--color-border)", padding: 12, overflowY: "auto" };
  const cell: React.CSSProperties = { border: "1px solid var(--color-border)", padding: "4px 8px", fontSize: 12, textAlign: "left", whiteSpace: "nowrap", maxWidth: 240, overflow: "hidden", textOverflow: "ellipsis" };
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", height: "100vh", display: "flex" }}>
      <div style={sidebar}>
        ...
        {tables.map((t) => (
          <Button key={t.name} type="button" onClick={() => openTable(t.name)} aria-current={active === t.name ? "true" : undefined} style={{ width: "100%", border: "none", textAlign: "left", font: "inherit", padding: "5px 8px", borderRadius: 6, cursor: "pointer", fontSize: 13, background: active === t.name ? "color-mix(in srgb, var(--color-primary) 8%, transparent)" : "transparent", display: "flex", justifyContent: "space-between" }}>
      ...
      <div style={{ flex: 1, padding: 16, overflow: "auto" }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          <Button onClick={() => setTab("browse")} style={{ padding: "6px 14px", borderRadius: 6, border: "1px solid var(--color-border)", background: tab === "browse" ? "var(--color-primary, #2563eb)" : "var(--color-surface, #fff)", color: tab === "browse" ? "var(--color-surface, #fff)" : "var(--color-fg)" }}>閲覧</Button>
          <Button onClick={() => setTab("sql")} style={{ padding: "6px 14px", borderRadius: 6, border: "1px solid var(--color-border)", background: tab === "sql" ? "var(--color-primary, #2563eb)" : "var(--color-surface, #fff)", color: tab === "sql" ? "var(--color-surface, #fff)" : "var(--color-fg)" }}>SQL 実行</Button>
          <Button onClick={() => setTab("schema")} style={{ padding: "6px 14px", borderRadius: 6, border: "1px solid var(--color-border)", background: tab === "schema" ? "var(--color-primary, #2563eb)" : "var(--color-surface, #fff)", color: tab === "schema" ? "var(--color-surface, #fff)" : "var(--color-fg)" }}>スキーマ操作</Button>
        </div>
            ...
            <div>
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={{ borderCollapse: "collapse" }}>
                  <thead><tr>{columns.map((c) => <th key={c.column} style={{ ...cell, background: "var(--color-surface)", fontWeight: 600 }}>{c.column}<div style={{ fontSize: 10, color: "var(--color-muted, #aaa)", fontWeight: 400 }}>{c.type}</div></th>)}</tr></thead>
                  <tbody>{rows.map((row, i) => <tr key={i}>{columns.map((c) => <td key={c.column} style={cell}>{row[c.column] === null ? <span style={{ color: "var(--color-border)" }}>NULL</span> : String(row[c.column])}</td>)}</tr>)}</tbody>
                </table>
              </div>
            </div>
          )
        ...
        {tab === "sql" && (
          <div>
            <Textarea aria-label="実行する SQL" value={sql} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setSql(e.target.value)} rows={5} style={{ width: "100%", boxSizing: "border-box", fontFamily: "var(--font-mono)", fontSize: 13, padding: 10, border: "1px solid var(--color-border)", borderRadius: 8 }} />
              ...
              <div style={{ marginTop: 12 }}>
                {sqlResult.error ? <p style={{ color: "var(--color-danger, #c00)", fontSize: 13 }}>{sqlResult.error}</p> : sqlResult.rows ? (
                  <div style={{ overflowX: "auto" }}>
                    ...
                    <table style={{ borderCollapse: "collapse" }}>
                      <thead><tr>{Object.keys(sqlResult.rows[0] ?? {}).map((k) => <th key={k} style={{ ...cell, background: "var(--color-surface)" }}>{k}</th>)}</tr></thead>
        ...
        {tab === "schema" && (
          <div style={{ maxWidth: 640 }}>
            <div style={{ background: "var(--color-surface, #fff)", border: "1px solid var(--color-border)", borderRadius: 10, padding: 16, marginBottom: 16 }}>
              <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>テーブル作成</div>
              <Input value={newTable} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewTable(e.target.value)} placeholder="テーブル名" style={{ padding: "6px 10px", border: "1px solid var(--color-border)", borderRadius: 6, marginBottom: 8, width: 240 }} />
              {newCols.map((c, i) => (
                <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                  <Input value={c.name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewCols(newCols.map((x, j) => j === i ? { ...x, name: e.target.value } : x))} placeholder="カラム名" style={{ padding: "5px 8px", border: "1px solid var(--color-border)", borderRadius: 6, flex: 1 }} />
                  <Input value={c.type} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewCols(newCols.map((x, j) => j === i ? { ...x, type: e.target.value } : x))} placeholder="型（text, integer...）" style={{ padding: "5px 8px", border: "1px solid var(--color-border)", borderRadius: 6, flex: 1 }} />
              ...
              <Button onClick={() => setNewCols([...newCols, { name: "", type: "text" }])} style={{ fontSize: 12, background: "none", border: "1px dashed var(--color-border)", borderRadius: 6, padding: "4px 12px", cursor: "pointer" }}>+ カラム追加</Button>
            ...
            {active !== "" && (
              <div style={{ background: "var(--color-surface, #fff)", border: "1px solid var(--color-border)", borderRadius: 10, padding: 16, marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                  <Input value={colName} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setColName(e.target.value)} placeholder="新カラム名" style={{ padding: "5px 8px", border: "1px solid var(--color-border)", borderRadius: 6, flex: 1 }} />
                  <Input value={colType} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setColType(e.target.value)} placeholder="型" style={{ padding: "5px 8px", border: "1px solid var(--color-border)", borderRadius: 6, flex: 1 }} />
