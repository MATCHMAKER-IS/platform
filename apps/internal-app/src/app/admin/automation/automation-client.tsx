    ...
    <div className="mx-auto max-w-3xl p-6">
      ...
      {tab === "export" && (
        <div>
          <div className="mb-4 flex items-end gap-2 rounded border border-neutral-200 p-3">
            <label className="text-xs text-neutral-500">種別<select value={newType} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setNewType(e.target.value)} className="mt-0.5 block rounded border border-neutral-300 px-2 py-1 text-sm"><option value="backup">バックアップ</option><option value="partners">取引先</option><option value="invoices">請求</option><option value="audit">監査ログ</option></select></label>
            <label className="text-xs text-neutral-500">頻度<select value={newFreq} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setNewFreq(e.target.value)} className="mt-0.5 block rounded border border-neutral-300 px-2 py-1 text-sm"><option value="daily">毎日</option><option value="weekly">毎週</option><option value="monthly">毎月</option></select></label>
      ...
      {tab === "reports" && (
        <div>
          ...
          <div className="mb-4 flex items-end gap-2 rounded border border-neutral-200 p-3">
            <label className="text-xs text-neutral-500">レポート<select value={rType} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setRType(e.target.value)} className="mt-0.5 block rounded border border-neutral-300 px-2 py-1 text-sm"><option value="sales">売上</option><option value="receivables">売掛</option><option value="inventory">在庫</option></select></label>
            <label className="text-xs text-neutral-500">頻度<select value={rFreq} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setRFreq(e.target.value)} className="mt-0.5 block rounded border border-neutral-300 px-2 py-1 text-sm"><option value="daily">毎日</option><option value="weekly">毎週</option><option value="monthly">毎月</option></select></label>
