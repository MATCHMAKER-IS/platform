    ...
    <div className="mx-auto max-w-4xl p-6">
      ...
      {journal && (
        <div className="mb-6 rounded border border-[var(--color-border)] bg-[var(--color-subtle)] p-4">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {journal.rows.map((r, i) => <tr key={i} className="border-b border-[var(--color-border)]"><td className="px-2 py-1.5">{r.account}</td><td className="px-2 py-1.5 text-xs text-[var(--color-muted)]">{r.memo ?? ""}</td><td className="px-2 py-1.5 text-right">{r.debit ? yen(r.debit) : ""}</td><td className="px-2 py-1.5 text-right">{r.credit ? yen(r.credit) : ""}</td></tr>)}
              {journal.rows.length === 0 && <tr><td colSpan={4} className="px-2 py-3 text-center text-[var(--color-muted)]">当年の償却はありません。</td></tr>}
            </tbody>
          </table>
        </div>
      )}
      ...
      {data && (
        <div className="mb-4 grid grid-cols-3 gap-3 text-center text-sm">
          <div className="rounded border border-[var(--color-border)] p-3"><div className="text-xs text-[var(--color-muted)]">取得価額合計</div><div className="mt-1 font-bold">{yen(data.summary.totalCost)}</div></div>
          <div className="rounded border border-[var(--color-border)] p-3"><div className="text-xs text-[var(--color-muted)]">簿価合計（{data.year}年）</div><div className="mt-1 font-bold">{yen(data.summary.totalBookValue)}</div></div>
          <div className="rounded border border-[var(--color-border)] p-3"><div className="text-xs text-[var(--color-muted)]">償却累計</div><div className="mt-1 font-bold">{yen(data.summary.totalAccumulated)}</div></div>
        </div>
      )}
      ...
      {canWrite && (
        <div className="mb-6 rounded border border-[var(--color-border)] p-4">
          ...
          <div className="flex flex-wrap items-end gap-2">
            ...
            <label className="text-xs text-[var(--color-muted)]">取得日<Input type="date" value={form.acquiredOn} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, acquiredOn: e.target.value })} className="mt-0.5 block rounded border border-[var(--color-border)] px-2 py-1 text-sm" /></label>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {(data?.assets ?? []).map((a) => (
            <tr key={a.code} className="border-b border-[var(--color-border)]">
      ...
      {schedule && (
        <div className="mt-6 rounded border border-[var(--color-border)] p-4">
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b border-[var(--color-border)] text-left text-xs text-[var(--color-muted)]"><th className="px-2 py-1">年度</th><th className="px-2 py-1 text-right">償却額</th><th className="px-2 py-1 text-right">償却累計</th><th className="px-2 py-1 text-right">期末簿価</th></tr></thead>
            <tbody>
              {schedule.rows.map((r) => (
                <tr key={r.year} className="border-b border-[var(--color-border)]"><td className="px-2 py-1.5">{r.year}</td><td className="px-2 py-1.5 text-right">{yen(r.depreciation)}</td><td className="px-2 py-1.5 text-right">{yen(r.accumulated)}</td><td className="px-2 py-1.5 text-right">{yen(r.bookValue)}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
