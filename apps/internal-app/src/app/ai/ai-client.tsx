...
export function AiClient({ fetchImpl }: { fetchImpl?: typeof fetch }) {
  ...
  // **1 円未満まで出す。** トークン単価は円未満なので、
  // 他の画面の `yen`(桁区切り・小数なし)とは別物。名前で区別する(2026-08)
  const costYen = (n: number) => `¥${n.toFixed(4)}`;

  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 24, fontFamily: "system-ui, sans-serif" }}>
      <h1 style={{ fontSize: 22 }}>AI アシスタント（文書要約）</h1>
      {usage?.mock && <p style={{ fontSize: 12, color: "var(--color-warning, #b45309)", background: "var(--color-bg)", padding: "8px 12px", borderRadius: 8 }}>モックモードで動作中です（ANTHROPIC_API_KEY 未設定）。要約はダミー文言ですが、コスト集計の流れは同じです。</p>}

      <div style={{ ...card, marginTop: 16 }}>
        <Textarea value={text} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setText(e.target.value)} placeholder="要約したい文章を貼り付けてください" rows={8} style={{ width: "100%", boxSizing: "border-box", padding: 10, border: "1px solid var(--color-border)", borderRadius: 8, fontFamily: "inherit" }} />
        <div style={{ display: "flex", gap: 12, alignItems: "center", marginTop: 8 }}>
          ...
          <Button onClick={summarize} disabled={text.trim().length === 0} style={{ marginLeft: "auto", padding: "8px 20px", background: busy ? "var(--color-border)" : "var(--color-primary, #2563eb)", color: "var(--color-surface, #fff)", border: "none", borderRadius: 8, cursor: busy ? "default" : "pointer" }}>{busy ? "要約中…" : "要約する"}</Button>
        </div>
      ...
      {result && (
        <div style={{ ...card, marginTop: 12 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--color-muted, #888)", marginTop: 10, borderTop: "1px solid #f0f0f0", paddingTop: 8 }}>
            トークン: 入力 {result.usage.inputTokens} / 出力 {result.usage.outputTokens}　コスト: {result.costJpy !== null ? costYen(result.costJpy) : "—"}
          </div>
        </div>
      )}
      ...
      {usage && (
        <div style={{ ...card, marginTop: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 10, marginBottom: 12 }}>
            ...
            <div><div style={{ fontSize: 11, color: "var(--color-muted, #888)" }}>累計コスト</div><div style={{ fontSize: 18, fontWeight: 600 }}>{costYen(usage.totals.costJpy)}</div></div>
            <div>
              <div style={{ fontSize: 11, color: "var(--color-muted, #888)" }}>入力トークン</div>
              <div style={{ fontSize: 18, fontWeight: 600 }}>{usage.totals.inputTokens.toLocaleString("ja-JP")}</div></div>
            <div>
              <div style={{ fontSize: 11, color: "var(--color-muted, #888)" }}>出力トークン</div>
              <div style={{ fontSize: 18, fontWeight: 600 }}>{usage.totals.outputTokens.toLocaleString("ja-JP")}</div></div>
          </div>
          {usage.recent.length > 0 && (
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead><tr style={{ color: "var(--color-muted, #888)", textAlign: "left", borderBottom: "1px solid var(--color-border)" }}><th style={{ padding: 4 }}>時刻</th><th>利用者</th><th>モデル</th><th>遅延</th><th>コスト</th></tr></thead>
              <tbody>
                {usage.recent.map((r, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid var(--color-surface)" }}>
                    ...
                    <td>{r.costJpy !== null ? costYen(r.costJpy) : (r.ok ? "—" : "失敗")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
