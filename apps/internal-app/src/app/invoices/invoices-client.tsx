...
const STATUS: Record<string, { label: string; cls: string }> = {
  ...
  overdue: { label: "期限超過", cls: "bg-[color-mix(in_srgb,var(--color-danger)_15%,transparent)] text-[var(--color-danger)]" },
  paid: { label: "入金済", cls: "bg-[color-mix(in_srgb,var(--color-success)_15%,transparent)] text-[var(--color-success)]" },
      ...
      {rcv && rcv.outstanding > 0 && (
        <div className="mb-6 rounded border border-[var(--color-border)] p-4">
          ...
          <div className="grid grid-cols-5 gap-2 text-center text-xs">
            <div className="rounded bg-[var(--color-subtle)] p-2"><div className="text-[var(--color-muted)]">期限前</div><div className="font-medium">{yen(rcv.aging.current)}</div></div>
            <div className="rounded bg-[color-mix(in_srgb,var(--color-warning)_8%,transparent)] p-2"><div className="text-[var(--color-warning)]">1〜30日</div><div className="font-medium">{yen(rcv.aging.d1_30)}</div></div>
            <div className="rounded bg-[color-mix(in_srgb,var(--color-warning)_15%,transparent)] p-2"><div className="text-[var(--color-warning)]">31〜60日</div><div className="font-medium">{yen(rcv.aging.d31_60)}</div></div>
            <div className="rounded bg-[color-mix(in_srgb,var(--color-danger)_8%,transparent)] p-2"><div className="text-[var(--color-danger)]">61〜90日</div><div className="font-medium">{yen(rcv.aging.d61_90)}</div></div>
            <div className="rounded bg-[color-mix(in_srgb,var(--color-danger)_15%,transparent)] p-2"><div className="text-[var(--color-danger)]">90日超</div><div className="font-medium">{yen(rcv.aging.over90)}</div></div>
          </div>
          {rcv.dunning.length > 0 && (
            <div className="mt-3">
              <h3 className="mb-1 text-xs font-medium text-[var(--color-muted)]">督促対象 {rcv.dunning.length} 件</h3>
      ...
      {creating && (
        <div className="mb-6 rounded border border-[var(--color-border)] p-4">
          <div className="mb-3 grid gap-2 md:grid-cols-2">
            <Input value={header.number} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeader({ ...header, number: e.target.value })} placeholder="請求書番号" className="rounded border border-[var(--color-border)] px-2 py-1 text-sm" />
            {customers.length > 0 && (
              <Select
                value="" onChange={(e: React.ChangeEvent<HTMLSelectElement>) => { const c = customers.find((x) => x.code === e.target.value); if (c) setHeader({ ...header, billTo: c.name }); }} className="rounded border border-[var(--color-border)] px-2 py-1 text-sm"
                options={[
                  { label: "取引先から選択…", value: "" }, ...customers.map((c) => ({ label: c.name, value: String(c.code) })),
                ]}
              />
            )}
            <Input value={header.billTo} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeader({ ...header, billTo: e.target.value })} placeholder="宛先" className="rounded border border-[var(--color-border)] px-2 py-1 text-sm" />
            <label className="text-xs text-[var(--color-muted)]">発行日<Input type="date" value={header.issueDate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeader({ ...header, issueDate: e.target.value })} className="mt-0.5 block w-full rounded border border-[var(--color-border)] px-2 py-1 text-sm" /></label>
            <label className="text-xs text-[var(--color-muted)]">支払期限<Input type="date" value={header.dueDate} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeader({ ...header, dueDate: e.target.value })} className="mt-0.5 block w-full rounded border border-[var(--color-border)] px-2 py-1 text-sm" /></label>
            <Input value={header.registrationNumber} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeader({ ...header, registrationNumber: e.target.value })} placeholder="登録番号 T+13桁（任意）" className="rounded border border-[var(--color-border)] px-2 py-1 text-sm md:col-span-2" />
          </div>
          <table className="mb-2 w-full text-sm">
            <thead><tr className="text-left text-xs text-[var(--color-muted)]"><th className="px-1 py-1">摘要</th><th className="px-1 py-1 w-20">数量</th><th className="px-1 py-1 w-28">単価</th><th className="px-1 py-1 w-20">税率</th><th></th></tr></thead>
            <tbody>
              {lines.map((l, i) => (
                <tr key={i}>
        ...
        <tbody>
          {invoices.map((inv) => (
            <tr key={inv.number} className="border-b border-[var(--color-border)]">
              ...
              <td className="px-2 py-2"><span className={`rounded px-2 py-0.5 text-xs ${STATUS[inv.status]?.cls ?? "bg-[var(--color-subtle)]"}`}>{STATUS[inv.status]?.label ?? inv.status}</span></td>
