  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: 24, fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ ...card, marginTop: 12 }}>
        <Input value={title} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTitle(e.target.value)} placeholder="タイトル（例: 4月定例MTG 議事録）" style={{ width: "100%", boxSizing: "border-box", padding: 8, border: "1px solid var(--color-border)", borderRadius: 6, marginBottom: 8 }} />
        <Textarea value={text} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setText(e.target.value)} placeholder="文字起こしテキストを貼り付け" rows={6} style={{ width: "100%", boxSizing: "border-box", padding: 8, border: "1px solid var(--color-border)", borderRadius: 6, fontFamily: "inherit" }} />
        <div style={{ display: "flex", gap: 12, alignItems: "center", marginTop: 8 }}>
          ...
          <Button onClick={submit} disabled={title.trim().length === 0 || text.trim().length === 0} style={{ marginLeft: "auto", padding: "8px 20px", background: busy ? "var(--color-border)" : "var(--color-primary, #2563eb)", color: "var(--color-surface, #fff)", border: "none", borderRadius: 8 }}>{busy ? "取り込み中…" : "補正して取り込む"}</Button>
        </div>
        ...
        <div style={{ ...card, marginTop: 12 }}>
          ...
          {result.changed && (
            <div>
              ...
              <pre style={{ fontSize: 12, background: "var(--color-bg)", padding: 12, borderRadius: 8, whiteSpace: "pre-wrap", lineHeight: 1.6 }}>{result.corrected}</pre>
            </div>
          )}
        </div>
      )}
