  ...
  return (
      ...
      <section className="mb-6 rounded border border-[var(--color-border)] p-4">
        ...
        <ul className="mt-2 list-disc pl-5 text-sm">
          <li><a href="/api/v1/openapi" className="text-[var(--color-primary)] hover:underline">/api/v1/openapi</a>（OpenAPI 3.0 JSON）</li>
          <li><code>GET /api/v1/invoices</code> — 請求一覧（スコープ <code>invoice:read</code>・100回/分）</li>
        </ul>
        <pre className="mt-2 overflow-x-auto rounded bg-[var(--color-fg)] p-3 text-xs text-[var(--color-primary-fg)]">{`curl -H "Authorization: Bearer sk_live_..." \\
  ${typeof window !== "undefined" ? window.location.origin : ""}/api/v1/invoices`}</pre>
      </section>

      <section className="mb-6 rounded border border-[var(--color-border)] p-4">
        <h2 className="mb-2 text-lg font-semibold">APIキーの発行</h2>
        <ol className="list-decimal pl-5 text-sm text-[var(--color-fg)]">
          <li>管理者で <a href="/admin/service-accounts" className="text-[var(--color-primary)] hover:underline">/admin/service-accounts</a> を開く</li>
