  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: 24, fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ ...card, marginTop: 12, display: "flex", gap: 8 }}>
        <Button onClick={() => run(false)} style={{ padding: "8px 20px", background: busy ? "var(--color-border)" : "var(--color-primary, #2563eb)", color: "var(--color-surface, #fff)", border: "none", borderRadius: 8 }}>{busy ? "実行中…" : "成功パターンを実行"}</Button>
        <Button onClick={() => run(true)} style={{ padding: "8px 20px", background: "var(--color-surface, #fff)", color: "var(--color-danger, #c00)", border: "1px solid #f0c0c0", borderRadius: 8 }}>失敗パターンを実行（リトライ確認）</Button>
      </div>
      {result && (
        <div style={{ ...card, marginTop: 12, background: result.ok ? "var(--color-bg)" : "var(--color-bg)" }}>
      ...
      <div style={{ ...card, marginTop: 12 }}>
        ...
        {events.map((e, i) => (
          <div key={i} style={{ fontSize: 12, padding: "4px 0", borderBottom: "1px solid var(--color-surface)", display: "flex", gap: 8 }}>
            ...
            <code style={{ color: e.action.includes("error") ? "var(--color-danger, #c00)" : e.action.includes("success") ? "var(--color-success, #16a34a)" : "var(--color-fg)" }}>{e.action}</code>
