  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      ...
      {canWrite && (
        <div className="mb-6 rounded border border-[var(--color-border)] p-4">
          ...
          <div className="flex flex-wrap items-end gap-2">
            <label className="text-xs text-[var(--color-muted)]">支払先<Input value={form.payee} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setForm({ ...form, payee: e.target.value })} className="mt-0.5 block rounded border border-[var(--color-border)] px-2 py-1 text-sm" /></label>
      ...
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className="text-xs text-[var(--color-muted)]">源泉徴収税 合計 {yen(totalWh)}</span>
      </div>
      <table className="w-full text-sm">
        ...
        <tbody>
          {(data?.report ?? []).map((r) => (
            <tr key={`${r.payee}:${r.category}`} className="border-b border-[var(--color-border)]">
              <td className="px-2 py-2">{r.payee}</td>
              <td className="px-2 py-2 text-xs text-[var(--color-muted)]">{r.category}</td>
