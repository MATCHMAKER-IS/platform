...
const STATUS: Record<string, { label: string; cls: string }> = {
  ...
  sent: { label: "送付済", cls: "bg-[color-mix(in_srgb,var(--color-primary)_15%,transparent)] text-[var(--color-primary)]" },
  accepted: { label: "受注", cls: "bg-[color-mix(in_srgb,var(--color-success)_15%,transparent)] text-[var(--color-success)]" },
  ...
  return (
    <div className="mx-auto max-w-5xl p-6">
      ...
      {creating && (
        <div className="mb-6 rounded border border-[var(--color-border)] p-4">
          <div className="mb-3 grid gap-2 md:grid-cols-2">
            <Input value={header.number} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeader({ ...header, number: e.target.value })} placeholder="見積番号" className="rounded border border-[var(--color-border)] px-2 py-1 text-sm" />
            ...
            <label className="text-xs text-[var(--color-muted)]">有効期限<Input type="date" value={header.validUntil} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setHeader({ ...header, validUntil: e.target.value })} className="mt-0.5 block w-full rounded border border-[var(--color-border)] px-2 py-1 text-sm" /></label>
          </div>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {quotes.map((q) => (
            <tr key={q.number} className="border-b border-[var(--color-border)]">
              <td className="px-2 py-2 font-mono text-xs">{q.number}</td>
              <td className="px-2 py-2">{q.billTo}</td>
              <td className="px-2 py-2 text-xs text-[var(--color-muted)]">{q.validUntil}{q.status !== "expired" && q.status !== "accepted" && q.daysLeft >= 0 && <span className="ml-1">（残{q.daysLeft}日）</span>}</td>
              <td className="px-2 py-2 text-right font-medium">{yen(q.totals.total)}</td>
              <td className="px-2 py-2"><span className={`rounded px-2 py-0.5 text-xs ${STATUS[q.status]?.cls ?? "bg-[var(--color-subtle)]"}`}>{STATUS[q.status]?.label ?? q.status}</span></td>
