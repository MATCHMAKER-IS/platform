    ...
    <div className="mx-auto max-w-5xl p-6">
      ...
      {freee && (
        <div className="mb-6 rounded border border-[var(--color-border)] bg-[var(--color-subtle)] p-4 text-sm">
          <p>freee 送信用バッチ：全 {freee.summary.total} 件中 <span className="font-medium text-[var(--color-success)]">{freee.summary.ready} 件が送信可能</span>{freee.summary.errors > 0 && <span className="text-[var(--color-danger)]">、{freee.summary.errors} 件は勘定科目の対応付けが必要</span>}。</p>
          {freee.errors.length > 0 && <ul className="mt-2 list-disc pl-5 text-xs text-[var(--color-danger)]">{freee.errors.map((e) => <li key={e.key}>{e.key}：未対応科目 {e.unknownAccounts.join("、")}</li>)}</ul>}
          <p className="mt-2 text-xs text-[var(--color-muted)]">※ 実際の送信には freee の勘定科目 ID の対応付けと認証が必要です（ここではプレビューのみ）。</p>
        </div>
      )}
      ...
      <div className="mb-6 rounded border border-[var(--color-border)] p-4">
        <div className="mb-2 flex items-center justify-between">
          ...
          {ledger.balanced ? <span className="rounded bg-[color-mix(in_srgb,var(--color-success)_15%,transparent)] px-2 py-0.5 text-xs text-[var(--color-success)]">貸借一致</span> : <span className="rounded bg-[color-mix(in_srgb,var(--color-danger)_15%,transparent)] px-2 py-0.5 text-xs text-[var(--color-danger)]">不一致</span>}
        </div>
        <table className="w-full text-sm">
          <thead><tr className="border-b border-[var(--color-border)] text-left text-xs text-[var(--color-muted)]"><th className="px-2 py-1">勘定科目</th><th className="px-2 py-1 text-right">借方</th><th className="px-2 py-1 text-right">貸方</th><th className="px-2 py-1 text-right">残高</th></tr></thead>
          <tbody>
        ...
        <tbody>
          {ledger.rows.map((r, i) => (
            <tr key={i} className="border-b border-[var(--color-border)]">
              <td className="px-2 py-1.5 text-xs text-[var(--color-muted)]">{r.date}</td>
          ...
          {ledger.rows.length === 0 && <tr><td colSpan={5} className="px-2 py-4 text-center text-sm text-[var(--color-muted)]">仕訳がありません。</td></tr>}
        </tbody>
      </table>
      ...
      <div className="mt-6 rounded border border-[var(--color-border)] p-4">
        ...
        {showAccounts && (
          <>
            <p className="mb-2 text-xs text-[var(--color-muted)]">ここで科目の区分（資産・負債・純資産・収益・費用）を登録すると、手動仕訳の任意科目も損益計算書・貸借対照表に集計されます。</p>
