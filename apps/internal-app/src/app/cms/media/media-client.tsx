    ...
    <div className="mx-auto max-w-4xl p-6">
      ...
      {media.length === 0 ? (
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
          {media.map((m) => (
            <figure key={m.key} className="overflow-hidden rounded border border-[var(--color-border)]">
              <img src={m.url} alt={m.name} className="h-32 w-full object-cover" loading="lazy" />
              <figcaption className="truncate px-2 py-1 text-xs text-[var(--color-muted)]" title={m.name}>{m.name}</figcaption>
            </figure>
          ))}
        </div>
      )}
