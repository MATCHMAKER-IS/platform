...
export function PayablesClient({ fetchImpl, canPay = true }: PayablesClientProps) {
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      ...
      <p className="mb-4 text-xs text-[var(--color-muted)]">発注に対する未払と支払予定です。支払を記録すると未払残に反映されます。</p>
            ...
            <tbody>
              {summary.upcoming.map((d) => (
                <tr key={d.number} className="border-b border-[var(--color-border)]">
                  ...
                  <td className="px-2 py-2 text-right">{canPay && <Button onClick={() => pay(d)} className="text-[var(--color-primary)] hover:underline">支払記録</Button>}</td>
                </tr>
              ))}
