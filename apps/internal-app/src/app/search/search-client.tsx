  ...
  return (
    <div className="mx-auto max-w-2xl p-6">
      ...
      <div className="flex gap-2">
        <Input value={q} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setQ(e.target.value)} onKeyDown={(e: React.KeyboardEvent) => { if (e.key === "Enter") void run(); }} placeholder="請求番号・取引先名・操作など" className="flex-1 rounded border border-[var(--color-border)] px-3 py-2 text-sm" />
        <Button onClick={() => void run()} className="rounded bg-[var(--color-fg)] px-5 py-2 text-sm text-white">検索</Button>
      </div>
      {searched && (
        <div className="mt-3 flex gap-1">
          {types.map((t) => <Button key={t} onClick={() => setFilter(t)} className={`rounded px-2 py-1 text-xs ${filter === t ? "bg-[var(--color-fg)] text-white" : "bg-[var(--color-subtle)] text-[var(--color-muted)]"}`}>{t === "all" ? "すべて" : TYPE_LABEL[t]}</Button>)}
        </div>
      )}
