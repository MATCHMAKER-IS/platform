...
export function AiImageClient({ fetchImpl }: { fetchImpl?: typeof fetch }) {
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: 24, fontFamily: "system-ui, sans-serif" }}>
      ...
      {result?.mock && <p style={{ fontSize: 12, color: "var(--color-warning, #b45309)", background: "var(--color-bg)", padding: "8px 12px", borderRadius: 8 }}>モックモードで動作中です（OPENAI_API_KEY 未設定）。プレースホルダ画像を返しますが、Gateway 経由・コスト計上の流れは同じです。</p>}
      <div style={{ ...card, marginTop: 12 }}>
        <Input value={prompt} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPrompt(e.target.value)} placeholder="例: 会社ロゴを水彩風に / 青空の背景" style={{ width: "100%", boxSizing: "border-box", padding: "8px 12px", border: "1px solid var(--color-border)", borderRadius: 8 }} />
        <Button onClick={generate} disabled={prompt.trim().length === 0} style={{ marginTop: 8, padding: "8px 20px", background: busy ? "var(--color-border)" : "var(--color-primary, #2563eb)", color: "var(--color-surface, #fff)", border: "none", borderRadius: 8 }}>{busy ? "生成中…" : "生成する"}</Button>
      ...
      {result && (
        <div style={{ ...card, marginTop: 12 }}>
          ...
          {result.images.map((src, i) => <img loading="lazy" decoding="async" key={i} src={src} alt="生成画像" style={{ maxWidth: "100%", borderRadius: 8, border: "1px solid var(--color-border)" }} />)}
        </div>
      )}
    </div>
  );
