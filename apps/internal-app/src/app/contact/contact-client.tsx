    ...
    <div className="mx-auto max-w-xl p-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        <label className="text-sm"><span className="inline-flex items-center gap-1">カテゴリ <InfoTip text="お問い合わせの種類を選ぶと、担当部署へ振り分けられます。急ぎの不具合は「システム不具合」を選んでください。" /></span>
          <select value={form.category} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setForm({ ...form, category: e.target.value })} className="mt-0.5 block w-full rounded border border-neutral-300 px-2 py-1.5 text-sm">{CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select>
        </label>
