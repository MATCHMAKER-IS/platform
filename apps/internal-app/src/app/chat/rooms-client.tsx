...
export function RoomsClient({ roomIds, roomName, onOpen, fetchImpl }: RoomsClientProps) {
  ...
  return (
    <List>
      {rows.map((r) => (
    <Button key={r.roomId} onClick={() => onOpen?.(r.roomId)} variant="secondary" className="flex w-full items-center justify-between px-3 py-2 text-left hover:bg-[var(--color-muted-bg)]">
