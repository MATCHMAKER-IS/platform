...
const COLOR: Record<Tok["t"], string> = { comment: "var(--color-muted)", string: "var(--color-danger)", keyword: "var(--color-primary)", number: "var(--color-primary)", plain: "inherit" };
  ...
  return (
    <div style={{ border: "1px solid var(--color-border)", borderRadius: "var(--radius)", overflow: "hidden", background: "var(--color-bg)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 12px", borderBottom: "1px solid var(--color-border)", background: "var(--color-surface)" }}>
        <span style={{ fontSize: 11, color: "var(--color-muted)", fontFamily: "var(--font-mono)", textTransform: "uppercase" }}>{lang}</span>
      ...
      <pre style={{ margin: 0, overflowX: "auto", fontSize: 12.5, lineHeight: 1.7, fontFamily: "var(--font-mono)", padding: "10px 0" }}>
        <code>
          {lines.map((line, i) => (
            <div
              key={i}
