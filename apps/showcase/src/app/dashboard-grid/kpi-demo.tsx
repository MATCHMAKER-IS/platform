...
// **引数は「万円」。** 他の画面の `yen` は円を受けるので、
  // 同じ名前だと 10000 倍ずれる。名前で単位を示す(2026-08)
  const yenFromMan = (man: number) => formatYen(man * 10000);
  ...
  return (
    <>
      ...
      <DashboardGrid columns={12}>
        <DashboardWidget colSpan={3} bare><StatCard label="今月売上" value={yenFromMan(last.売上)} delta={pct(last.売上, prev.売上)} trend={trendOf(last.売上, prev.売上)} icon={<Icon name="TrendingUp" className="h-5 w-5" />} /></DashboardWidget>
        <DashboardWidget colSpan={3} bare><StatCard label="今月利益" value={yenFromMan(profit)} delta={pct(profit, prevProfit)} trend={trendOf(profit, prevProfit)} icon={<Icon name="Wallet" className="h-5 w-5" />} /></DashboardWidget>
        <DashboardWidget colSpan={3} bare><StatCard label={`平均売上(${months}ヶ月)`} value={yenFromMan(avgSales)} delta={`合計 ${yenFromMan(totalSales)}`} trend="flat" icon={<Icon name="ChartColumn" className="h-5 w-5" />} /></DashboardWidget>
        <DashboardWidget colSpan={3} bare><StatCard label="原価率" value={((last.費用 / last.売上) * 100).toFixed(1) + "%"} delta={`費用 ${yenFromMan(last.費用)}`} trend={trendOf(prev.費用 / prev.売上, last.費用 / last.売上)} icon={<Icon name="Receipt" className="h-5 w-5" />} /></DashboardWidget>
