"use client";
/** ダッシュボード の統合デモ（タブでまとめたもの）。 */
import * as React from "react";
import { Button } from "@platform/ui";
import { KpiDashboardDemo } from "./kpi-demo";
import { LiveDashboardDemo } from "./live-demo";
const TABS = [{ id: "a", label: "KPI・グラフ", Comp: KpiDashboardDemo }, { id: "b", label: "ライブ(DnD保存)", Comp: LiveDashboardDemo }] as const;
export default function Page() {
  const [tab, setTab] = React.useState<string>("a");
  const Active = (TABS.find((t) => t.id === tab) ?? TABS[0]).Comp;
  return (
    <main style={{ maxWidth: 1100, margin: "2rem auto", padding: "0 1rem" }}>
      <h1 style={{ fontSize: "1.5rem", fontWeight: 700, marginBottom: 12 }}>ダッシュボード</h1>
