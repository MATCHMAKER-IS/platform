...
// **ハッシュ連鎖とタイムスタンプはサーバ**（`/api/dencho-demo`）。
// `node:crypto` を使うのでブラウザでは動かず、それ以前に
// **「改ざんされていません」を画面が言うだけなら、その画面ごと書き換えられます**。
// 検索と保存期限は**計算だけ**なので画面でよい（入口を経由しないサブパスから取る）
import {
  searchTransactions,
  meetsSearchRequirements,
  type TransactionRecord,
  type TransactionQuery,
} from "@platform/dencho/search";
import {
  retentionDeadline,
  isWithinRetention,
  daysUntilRetentionEnd,
  DEFAULT_RETENTION_YEARS,
} from "@platform/dencho/retention";
import type { EvidenceRecord } from "@platform/dencho/types";
...
/** サーバに指示を送る（**この画面はハッシュを計算しません**）。 */
async function call(body: unknown): Promise<Record<string, unknown>> {
  const res = await fetch("/api/dencho-demo", {
    method: "POST",
...
export function DenchoDemo() {
  const [chain, setChain] = React.useState<EvidenceRecord[]>([]);
  // 初期の連鎖はサーバに作ってもらう
  React.useEffect(() => {
    let alive = true;
    void (async () => {
      const r = await call({ action: "build", transactions: TXS });
      if (alive) setChain((r.chain as EvidenceRecord[] | undefined) ?? []);
    })();
    return () => { alive = false; };
  }, []);
  ...
  // **検証もサーバ。** 画面が「正しい」と言っても意味がありません
  const [verification, setVerification] = React.useState<{ valid: boolean; brokenAt?: number } | null>(null);
  React.useEffect(() => {
    if (chain.length === 0) { setVerification(null); return; }
    let alive = true;
    void (async () => {
      const r = await call({ action: "verify", chain });
      if (alive) setVerification((r.verification as { valid: boolean; brokenAt?: number } | undefined) ?? null);
    })();
    return () => { alive = false; };
  }, [chain]);
  ...
  React.useEffect(() => {
    let alive = true;
    void (async () => {
      const r = await call({ action: "stamp", transactions: [TXS[0]], secret });
      if (!alive) return;
      setToken(String(r.token ?? ""));
      setTokenOk(r.ok === true);
    })();
    return () => { alive = false; };
  }, [secret]);
  ...
  return (
    <>
            ...
            {chain.map((r) => {
              ...
              return (
                <tr key={r.seq} style={{ borderTop: "1px solid var(--color-border)", background: broken ? "color-mix(in srgb, var(--color-danger) 8%, transparent)" : "transparent" }}>
                  ...
                  <td style={{ padding: 4, fontFamily: "var(--font-mono)", color: "var(--color-muted)" }}>{short(r.prevHash)}</td>
                  <td style={{ padding: 4, fontFamily: "var(--font-mono)", color: "var(--color-muted)" }}>{short(r.hash)}</td>
                  <td style={{ padding: 4 }}>
                    <Button
      ...
      <div style={box}>
        ...
        <div style={{ fontSize: 12, fontFamily: "var(--font-mono)", background: "var(--color-bg)", padding: 10, borderRadius: "var(--radius)", lineHeight: 1.8, wordBreak: "break-all" }}>
