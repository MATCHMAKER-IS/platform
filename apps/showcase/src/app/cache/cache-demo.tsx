          ...
          {KEYS.map((k) => {
            ...
            return (
              <Button key={k} type="button" onClick={() => get(k)} disabled={loading === k}
                style={{ padding: "8px 12px", borderRadius: "var(--radius)", cursor: "pointer", border: "1px solid var(--color-border)", background: "var(--color-bg)", color: "var(--color-fg)", fontFamily: "var(--font-mono)", fontSize: 12, textAlign: "left", minWidth: 130 }}>
        ...
        {log.length === 0 ? (<div style={{ fontSize: 12, color: "var(--color-muted)" }}>同じキーを続けて押すとヒット、TTL 切れ後はまたミスになります。</div>) : (
          <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 4 }}>
            {log.map((it, i) => (<li key={i} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
              ...
              <span style={{ fontFamily: "var(--font-mono)" }}>{it.key}</span>
