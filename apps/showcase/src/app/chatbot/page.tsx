...
/** 会話の保存先。**デモなのでブラウザに置く**(サーバを持たない)。 */
const store = createWebStorage<Turn[]>({
  key: "demo-chatbot-turns",
  fallback: [],
  validate: (v): v is Turn[] => Array.isArray(v),
});
...
export default function Page() {
  ...
  const save = (next: Turn[]) => {
    setTurns(next);
    // **直近 40 件だけ残す。** 会話は際限なく増えるので、容量を超えると保存できなくなる
    store.set(next.slice(-40));
  };
