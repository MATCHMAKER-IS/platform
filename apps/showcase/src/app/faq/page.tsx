...
const FAQ: { cat: string; items: QA[] }[] = [
  ...
  { cat: "開発の進め方", items: [
    ...
    {
      q: "勤怠や有給の計算は基盤にありますか？",
      a: "あります。@platform/attendance が打刻から実労働・残業・深夜・休日・遅刻・早退を集計し、その結果をそのまま @platform/payroll に渡して金額にできます（時間の区分は勤怠、金額は給与、と役割を分けています）。有給は法定の付与日数・時効2年・古い付与からの消化・年5日の取得義務までを扱います。古い付与から消化するのは、新しい分から使うと古い分が時効で消えて実質的に日数を捨てることになるためです。動作は /attendance で確認できます。",
    },
    {
      q: "本番稼働したらDBのスキーマ変更はどうしますか？",
      a: "開発中は db push（履歴なし）ですが、本番にデータが入ったら『baseline』方式でマイグレーションへ切り替えます。migrate dev は既存DBのリセットを促すため本番では使わず、migrate diff で初期SQLを作り migrate resolve --applied で適用済みとして登録します。手順は docs/adr/0014-migration-baseline-on-production.md。",
    },
    {
      q: "バックアップは取れていますか？",
      a: "取得手順に加えて『戻せること』を重視しています。RPO 24時間・RTO 4時間を目標に、半年に1回の復元訓練を行い ops/drills/restore-drill.json に記録します。node tools/check-drill.mjs が間隔を見張り、未実施や期限切れを警告します。詳細は docs/ops/BACKUP_RESTORE.md。",
    },
    { q: "DB はどう扱いますか？", a: "多くのストアは memory 実装と prisma 実装の両方を持ちます。開発・テストは memory、本番は prisma に差し替えます。このデモサイトは DB を持たず、メモリ・モックで動きます。" },
    { q: "新しく入った人はどう学べばいいですか？", a: "トップページの『開発の流れ』を読み、docs/onboarding/04-task.md の実地課題（備品貸出の画面を1つ作る）に取り組みます。詰まった箇所は本人の理解不足ではなく基盤側の欠陥として扱い、手順書やエラーメッセージを直します。" },
    { q: "AI に開発を手伝ってもらえますか？", a: "はい。CLAUDE.md と docs/ai/patterns.md に作法・実装パターンがあり、基盤の検索用に MCP カタログも用意されています。" },
  ] },
  { cat: "認証・セキュリティ・運用", items: [
    {
      q: "利用者から「ログインできない」と言われたら？",
      a: "docs/ops/SUPPORT_GUIDE.md に対応の手順があります。まず「どこで止まっているか」を聞きます——打ち間違いなら再設定リンク（30分・1回のみ有効）、「試行回数が多すぎます」なら総当たり対策が働いた状態で15分待てば解けます、2要素のコードが通らないなら端末の時計のずれが原因のことが多く、急ぐなら予備コードです。解除を依頼されたときは、本人からの依頼かを必ず確かめてください（電話で解除させるのは実在する攻撃手口です）。",
    },
    {
      q: "退職者の権限はどう管理しますか？",
      a: "ADR 0017 で方針を定めています。退職・異動は最終出社日のうちに停止し、順序はセッション無効化→ログイン停止→権限削除です（権限だけ消してもセッションが生きていれば操作できるため）。半年に1回の棚卸しでは、業務を知る人が要否を判定し、判定者と日付を記録します。@platform/access-review の reviewAccess() が退職者に残った権限・期限切れ・期限の無い強い権限を一覧にします。「*」や pii:unmask のような強い権限は恒久的に付けず、必ず期限を決めます。",
    },
    ...
    {
      q: "Slack で承認できるようにしたいのですが",
      a: "/approval の「Slackで承認」タブが実例です。buildApprovalBlocks で承認/却下ボタン付きの通知を作り、parseInteraction で押下を受け取ります。要点は「押した人が承認してよいか」を必ず確かめることです（Slackの利用者IDを社内利用者に突き合わせ、@platform/auth の can で権限を判定）。これを省くとチャンネルにいる人なら誰でも承認できてしまいます。受信口では署名検証も必須で、Slackは3秒で切断するため重い処理はジョブキューへ回します。",
    },
    {
      q: "Slack や Notion とも連携できますか？",
    ...
    {
      q: "Microsoft 365 と連携できますか？",
      a: "できます。@platform/microsoft が Entra ID(旧 Azure AD)の OAuth と Microsoft Graph を扱います。Outlook のメール送信、予定の作成・参照、Teams 会議リンク、社員情報の取得に対応しています。テナントは必ず自社の ID を指定してください（common にすると他社のアカウントでもログインできてしまいます）。リフレッシュトークンは更新のたびに回転するため、onRefresh で保存し直す必要があります。疎通は /connect で確認できます。",
    },
    {
      q: "外部サービスの連携に何が必要か分かりません",
      a: "/connect（接続チェック）を開いてください。freee・Google・Stripe・PayPal・LINE・Slack・Zoho・Resend・Meilisearch など11サービスについて、必要な値・入手先・必要な権限を示し、その場で疎通テストができます。セッション鍵など自分で用意する秘密の生成もここで行えます。",
    },
    {
      q: "SSO（シングルサインオン）で気をつけることは？",
      a: "署名が正しいトークンでも、中身を見ないと通ってしまう攻撃があります。@platform/auth の verifyIdTokenClaims() が、宛先(aud・別アプリ向けの流用を防ぐ)・発行者(iss)・期限(exp)・テナント(tid・他社アカウントを防ぐ)・nonce・メール確認済みを検証します。署名の検証は別途必要です。利用者の識別にはメールではなく iss+sub を使ってください（メールは結婚や異動で変わり、退職者のメールが再利用されると別人に繋がります）。SAML は現時点で未対応です（OIDCで足りているため）。",
    },
    {
      q: "2要素認証やパスワード再設定はありますか？",
      a: "あります。/login のタブで試せます。2要素認証は TOTP（認証アプリの6桁コード）と予備コード（1回ずつ使える逃げ道）に対応し、端末の時計ずれは±30秒まで許容します。パスワード再設定は使い捨てリンク方式で、保存するのはハッシュだけ・30分で失効・1回だけ有効・未登録アドレスでも同じ応答（登録の有無を漏らさない）にしています。再設定後は既存セッションを無効化します。",
    },
    {
      q: "個人情報の削除を求められたら、帳簿はどうしますか？",
      a: "ADR 0018 で方針を定めています。法令の保存義務（会計帳簿は電子帳簿保存法で7年、賃金台帳は労基法で5年）が削除要求に優先しますが、黙って残さず本人に説明します。@platform/pii の decideErasure() が「法令の義務 → 自社の保持期間 → 削除可」の順で判断し、explainErasure() が説明文を作ります。説明には何件消して何件残したか、残した根拠、いつ消せるかを必ず含めます（「消せません」だけでは納得が得られないため）。全部消すのも全部残すのも、どちらかの違反になります。",
    },
    ...
    {
      q: "社内向けのチャットボットは作れますか？",
      a: "/chatbot がその実例です。@platform/ui の ChatWindow で会話し、発言のたびに社内資料を検索して回答します。毎回引き直すので会話が長くなっても入力トークンが膨らまず、履歴も直近だけを送るため費用が際限なく伸びません。回答には根拠の資料パスを添えます（裏取りできない回答は業務では使えないため）。AIの鍵が無い環境では資料の抜粋を返します。",
    },
    {
      q: "社内のやり方を調べたいときは？",
      a: "/assistant（社内資料アシスタント）で質問すると、手順書・規約・設計判断(ADR)から該当箇所を探して文脈を組み立てます。検索はブラウザ内で完結するため鍵は不要です。AIの鍵を設定していれば、その文脈をもとに回答まで生成します（@platform/ai のゲートウェイ経由なので費用も記録されます）。AIアシスタントからは MCP の search_docs で同じ資料を引けます。",
    },
    {
      q: "外部SaaSのAPIが変わったら気づけますか？",
      a: "tests/contracts/ に『こちらが依存しているフィールド』を契約として書き、node tools/check-contract.mjs で実装とのズレを検査します。週次CI(contract.yml)が本物のAPIに問い合わせて記録を更新するため、相手の変更を利用者より先に検知できます。手順は docs/ops/CONTRACT_TESTING.md。",
    },
