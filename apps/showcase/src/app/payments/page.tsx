...
export default function Page() {
  ...
  const log = (text: string, kind: Ev["kind"] = "ok") =>
    setEvents((e) => [{ at: new Date().toLocaleTimeString("ja-JP"), text, kind }, ...e].slice(0, 12));
  ...
  const refund = () => {
    ...
    log(`返金: ${formatYen(amt)}（累計 ${formatYen(total)} / ${formatYen(amount)}）`);
  };
      ...
      <div style={box}>
        ...
        {orderId && (
          <div style={{ ...mono, padding: 10, background: "var(--color-bg)", borderRadius: "var(--radius)", border: "1px solid var(--color-border)", marginBottom: 12 }}>
            {provider === "stripe" ? "payment_intent" : "order_id"}: {orderId} / {formatYen(amount)}
            {refunded > 0 && <> / 返金済 {formatYen(refunded)}</>} / status: <strong>{LABEL[status]}</strong>
          </div>
        )}
