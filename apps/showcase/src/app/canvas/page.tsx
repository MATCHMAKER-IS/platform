...
/** 図形の保存先。**デモなのでブラウザに置く**(サーバを持たない)。 */
const store = createWebStorage<Obj[]>({
  key: "demo-canvas-positions-v2",
  fallback: [],
  validate: (v): v is Obj[] => Array.isArray(v),
});
const W = 96, H = 44;
...
const COLORS = ["var(--color-primary)", "var(--color-success)", "var(--color-warning)", "var(--color-danger)", "#7c3aed"];
const DEFAULT: Obj[] = [
  { id: "a", label: "タスクA", color: "var(--color-primary)", x: 20, y: 20 , shape: "round" },
  { id: "b", label: "タスクB", color: "var(--color-success)", x: 180, y: 60 , shape: "round" },
  { id: "c", label: "メモ", color: "var(--color-warning)", x: 90, y: 150 , shape: "note" },
  { id: "d", label: "承認待ち", color: "var(--color-danger)", x: 260, y: 180 , shape: "circle" },
];
...
export default function Page() {
  ...
  // **`localStorage` を直接触らない**(ADR-0020)。基盤の使い方を見せる画面なので手本であるべき
  React.useEffect(() => {
    setObjs(store.get());
  }, []);

  const persist = (next: Obj[]) => {
    const r = store.set(next);
    // **保存の失敗を握らない。** 画面には残るのに次回は消えている、を防ぐ
    if (r.ok) { setSaved(true); window.setTimeout(() => setSaved(false), 1500); }
  };
    ...
    <main style={{ maxWidth: 780, margin: "2.5rem auto", padding: "0 1rem" }}>
          ...
          {objs.map((o) => (
            <Button
              ...
              style={{
                ...
                height: H,
                background: o.color, color: "var(--color-bg)",
      ...
      <div style={box}>
        ...
        <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "grid", gap: 4 }}>
          {objs.map((o) => (
            <li key={o.id} style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: selected === o.id ? "var(--color-fg)" : "var(--color-muted)" }}>
