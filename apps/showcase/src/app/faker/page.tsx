...
export default function Page() {
  ...
  // **CSV 化は基盤に任せる。** 以前ここで自作していたが、
  // 引用符の扱い・BOM・改行コードは `@platform/csv` が面倒を見ている。
  // 画面ごとに書くと、Excel で開けない CSV が 1 つずつ増えていく。
  const saveCsv = () => {
    downloadCsv(`dummy-${generatedWith ?? 0}.csv`, rows as unknown as Record<string, unknown>[], {
      columns: [
        { key: "name", header: "氏名" },
    ...
    <main style={{ maxWidth: 900, margin: "2.5rem auto", padding: "0 1rem" }}>
      ...
      <div style={box}>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
          ...
          <Button variant="secondary" onClick={saveCsv} disabled={rows.length === 0}>CSV で保存</Button>
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i} style={{ borderTop: "1px solid var(--color-border)" }}>
                  ...
                  <td style={{ ...td, fontFamily: "var(--font-mono)", fontSize: 11.5 }}>{r.mail}</td>
                  <td style={{ ...td, fontFamily: "var(--font-mono)", fontSize: 11.5 }}>{r.tel}</td>
                  <td style={{ ...td, fontFamily: "var(--font-mono)", fontSize: 11.5 }}>{r.zip}</td>
