            ...
            {lines.map((l, i) => (
              <tr key={i} style={{ borderTop: "1px solid var(--color-border)" }}>
                ...
                <td style={{ padding: 4, width: 60 }}>
                  <Input type="number" value={l.quantity} onChange={(e) => update(i, { quantity: parseNumberOr(e.target.value, 0) })} style={{ ...cell, textAlign: "right" }} />
                </td>
                <td style={{ padding: 4, width: 90 }}>
                  <Input type="number" value={l.unitPrice} onChange={(e) => update(i, { unitPrice: parseNumberOr(e.target.value, 0) })} style={{ ...cell, textAlign: "right" }} />
                </td>
                <td style={{ padding: 4, width: 80 }}>
                  <Input type="number" value={l.discount ?? 0} onChange={(e) => update(i, { discount: parseNumberOr(e.target.value, 0) })} style={{ ...cell, textAlign: "right" }} />
                </td>
                <td style={{ padding: 4, width: 80 }}>
                  <Select
                    value={String(l.taxRate ?? 10)} onChange={(e) => update(i, { taxRate: parseNumberOr(e.target.value, 0) as TaxRate })} style={cell}
                    options={[
                      { label: "10%", value: "10"},
                      {label: "8%", value: "8"},
                      {label: "0%", value: "0" },
      ...
      <div style={box}>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
          <label style={{ fontSize: 12 }}>
            ...
            <Input type="number" value={seq} onChange={(e) => setSeq(parseNumberOr(e.target.value, 0))} style={{ ...cell, width: 80, textAlign: "right" }} />
          </label>
          <label style={{ fontSize: 12 }}>
            <div style={{ marginBottom: 4, color: "var(--color-muted)" }}>支払条件</div>
            <Select
              value={termIndex}
              onChange={(e) => setTermIndex(parseNumberOr(e.target.value, 0))}
          ...
          <label style={{ fontSize: 12 }}>
            ...
            <Input type="number" value={paid} onChange={(e) => setPaid(parseNumberOr(e.target.value, 0))} style={{ ...cell, width: 110, textAlign: "right" }} />
          </label>
          <label style={{ fontSize: 12 }}>
      ...
      <div style={{ ...box, padding: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ textAlign: "right", fontSize: 13, lineHeight: 1.9 }}>
            ...
            <div style={{ fontFamily: "var(--font-mono)" }}>{invoice.registrationNumber}</div>
          </div>
        </div>
