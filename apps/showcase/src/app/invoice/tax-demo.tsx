                ...
                <tr key={i} style={{ borderTop: "1px solid var(--color-border)" }}>
                  ...
                  <td style={{ padding: 4, width: 100 }}>
                    <Input
                      type="number"
                      value={r.amount}
                      onChange={(e) => update(i, { amount: parseNumberOr(e.target.value, 0) })}
                      style={{ ...cell, textAlign: "right" }}
                    />
                  </td>
                  <td style={{ padding: 4, width: 80 }}>
                    <Select
                      value={String(r.rate)} onChange={(e) => update(i, { rate: parseNumberOr(e.target.value, 0) as TaxRate })} style={cell}
                      options={[
                        { label: "10%", value: "10"},
                        {label: "8%（軽減）", value: "8"},
                        {label: "0%（非課税）", value: "0" },
