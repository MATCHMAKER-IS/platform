 ...
 * この画面は鍵が動的(マスタの種類ごと)なので、
 * 都度 `createWebStorage` を作る。**作る手間は小さく、
 * 例外処理を各所に書くより安い**。
 */
/** 変更履歴の 1 件。 */
...
/**
 * 変更履歴の保存先。**鍵が固定なので使い回す**。
 *
 * **`unknown[]` にしない。** 読み出した値をそのまま `setHistory` へ渡すので、
 * 型が合わずに落ちる(2026-08 の型検査で発覚)。`validate` は
 * **形まで見る**——端末に残った古い形をそのまま信じない。
 */
const historyStore = createWebStorage<HistoryEntry[]>({
  key: "demo-master-history",
  fallback: [],
  validate: (v): v is HistoryEntry[] =>
    Array.isArray(v) &&
...
function storeFor<T>(key: string, fallback: T) {
...
function refRows(masterId?: string): Row[] {
  ...
  if (!m) return [];
  return storeFor(m.storageKey, m.seed).get();
}
...
export default function Page() {
  ...
  React.useEffect(() => { setHistory(historyStore.get()); }, []);
  React.useEffect(() => { historyStore.set(history); }, [history]);
  ...
  // マスタ切替時に該当データを読み込み
  React.useEffect(() => {
    setRows(storeFor(master.storageKey, master.seed).get());
    setQ("");
  }, [mid]);

  const persist = (next: Row[]) => { setRows(next); storeFor(master.storageKey, next).set(next); };
  ...
  const reassignAll = () => {
    ...
    for (const { master: m, field } of refsToMaster(master.id)) {
      ...
      storeFor(m.storageKey, mrows).set(mrows);
  ...
  const cell = (r: Row, f: Field) => f.type === "bool" ? (r[f.key] ? <Badge variant="success">{f.label.includes("販売") ? "販売中" : "有効"}</Badge> : <Badge variant="secondary">停止</Badge>)
    ...
    : f.type === "number" ? Number(r[f.key] ?? 0).toLocaleString("ja-JP") : String(r[f.key] ?? "");
          ...
          <tbody>
            {filtered.map((r) => (
              <tr key={r.id} style={{ borderTop: "1px solid var(--color-border)" }}>
                {master.fields.map((f) => <td key={f.key} style={{ ...td, ...(f.key === "code" ? { fontFamily: "var(--font-mono)" } : {}) }}>{cell(r, f)}</td>)}
                {isReferenced && (
              ...
              {referencingRows(master.id, String(related.id)).map(({ master: m, row }) => {
                ...
                return (
                  <li key={key} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, padding: "8px 10px", borderRadius: 6, background: "var(--color-bg)", border: "1px solid var(--color-border)" }}>
                    ...
                    <span style={{ fontFamily: "var(--font-mono)" }}>{String(row.code)}</span>
              ...
              <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 6, maxHeight: 320, overflowY: "auto" }}>
                {history.map((h, i) => (
                  <li key={i} style={{ fontSize: 12.5, padding: "8px 10px", borderRadius: 6, background: "var(--color-bg)", border: "1px solid var(--color-border)" }}>
                    <span style={{ color: "var(--color-muted)", fontFamily: "var(--font-mono)", marginRight: 8 }}>{new Date(h.at).toLocaleString("ja-JP", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })}</span>
                    {h.text}
                  </li>
                ))}
              </ul>
