...
const columns: DataTableColumn<(typeof EXPENSES)[number]>[] = [
  ...
  { key: "net", header: "税抜", sortable: true, align: "right", render: (r) => formatYen(r.net) },
  { key: "tax", header: "消費税", align: "right", render: (r) => `${formatYen(r.tax)}（${r.taxRate}%）` },
    ...
    <main style={{ maxWidth: 900, margin: "2.5rem auto", padding: "0 1rem" }}>
      ...
      <div style={box}>
        ...
        <table style={{ width: "100%", fontSize: 13, borderCollapse: "collapse" }}>
          <tbody>
            {[
              { k: "金額", v: fields.amount !== undefined ? formatYen(fields.amount) : "—", note: "「合計」を探す" },
          ...
          <div style={{ marginTop: 10 }}>
            ...
            {breakdown.map((b, i) => (
              <div key={i} style={{ ...mono, fontSize: 12 }}>
                {b.rate}% 対象 {formatYen(b.subtotal)} / 消費税 {formatYen((b.tax ?? 0))}
              </div>
            ))}
          </div>
        )}
        ...
        <Alert variant="info" title="「全角で読まれた」を押してください" style={{ marginTop: 12 }}>
          <span style={{ fontSize: 12, lineHeight: 1.8 }}>
            ...
            <a href="/invoice" style={{ color: "var(--color-primary)" }}>消費税・インボイス</a>のデモも参照）。
            <br />
              ...
              <tbody>
                {journal.lines.map((l, i) => (
                  <tr key={i} style={{ borderTop: "1px solid var(--color-border)" }}>
                    ...
                    <td style={{ padding: 5, textAlign: "right", ...mono }}>{l.debit > 0 ? formatYen(l.debit) : ""}</td>
                    <td style={{ padding: 5, textAlign: "right", ...mono }}>{l.credit > 0 ? formatYen(l.credit) : ""}</td>
                  </tr>
                ))}
                <tr style={{ borderTop: "2px solid var(--color-border)", fontWeight: 700 }}>
                  <td style={{ padding: 5 }}>合計</td>
                  <td style={{ padding: 5, textAlign: "right", ...mono }}>{formatYen(debitTotal(journal))}</td>
                  <td style={{ padding: 5, textAlign: "right", ...mono }}>{formatYen(creditTotal(journal))}</td>
                </tr>
              </tbody>
            </table>
      ...
      <div style={box}>
          ...
          {[
            ...
            <div key={x.label} style={{ border: "1px solid var(--color-border)", borderRadius: "var(--radius)", padding: 10 }}>
              ...
              <div style={{ fontSize: 12, lineHeight: 1.9 }}>
                <div>
                  収益 <b>{formatYen(x.pl.revenue)}</b>
                </div>
                <div>
                  費用 <b style={{ color: x.pl.expense === 0 ? "var(--color-danger)" : undefined }}>{formatYen(x.pl.expense)}</b>
                </div>
                <div style={{ borderTop: "1px solid var(--color-border)", marginTop: 4, paddingTop: 4 }}>
                  純利益 <b>{formatYen(x.pl.netIncome)}</b>
                </div>
              </div>
        ...
        <Alert variant="danger" title="未登録の科目は集計されません" style={{ marginTop: 10 }}>
          <span style={{ fontSize: 12, lineHeight: 1.8 }}>
            <strong>左は費用が {formatYen(plDefault.expense)} です。</strong>
