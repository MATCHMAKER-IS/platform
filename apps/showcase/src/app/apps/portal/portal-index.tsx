...
export function PortalIndex({ packages, totals }: PortalIndexProps) {
  ...
  return (
    <>
      <div style={banner}>
        全 <strong>{totals.packages}</strong> パッケージ・<strong>{totals.functions.toLocaleString("ja-JP")}</strong> 関数・
        <strong>{totals.types.toLocaleString("ja-JP")}</strong> 型。<strong>全関数に TSDoc 完備</strong>。
        <br />
      ...
      <main style={{ maxWidth: 1000, margin: "1.5rem auto", padding: "0 1rem" }}>
            ...
            {shown.map((p) => (
              <Link key={p.name} href={`/apps/portal/${p.name}`} style={card}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
                  <b style={{ fontSize: 13, fontFamily: "var(--font-mono)" }}>@platform/{p.name}</b>
        ...
        <p style={{ fontSize: 11.5, color: "var(--color-muted)", marginTop: 20, lineHeight: 1.7 }}>
          この画面自体が実物です（かつて <code>apps/platform-portal</code> という別アプリでしたが、showcase に統合しました）。AI から探す場合は
