 ...
 * 備品管理のデモ(モックデータ)。
 *
 * **実物は `apps/internal-app` の `/equipment`**(equipment-app は 2026-08 に統合済み)。
 * ここはモックデータでの再現で、DB を持たない。
 */
...
export function EquipmentDemo() {
  ...
  return (
    <div>
      <div style={banner}>
        これは <strong>デモ</strong> です。実物は <code>apps/internal-app</code> の <code>/equipment</code>。ここは<strong>モックデータ</strong>で、DB を使いません。
      </div>
          ...
          <tbody>
            {shown.map((i) => (
              <tr key={i.id} style={{ borderTop: "1px solid var(--color-border)" }}>
                <td style={{ ...td, fontFamily: "var(--font-mono)" }}>{i.id}</td>
                <td style={td}>{i.name}</td>
                <td style={td}>{i.category}</td>
                <td style={td}>
                  <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 999, color: "var(--color-bg)", background: STATUS_COLOR[i.status] }}>
                    {i.status}
                  </span>
                </td>
