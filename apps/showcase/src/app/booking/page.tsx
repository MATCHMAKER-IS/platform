...
/** 予約の保存先。**デモなのでブラウザに置く**(サーバを持たない)。 */
const store = createWebStorage<Reservation[]>({
  key: "demo-booking-v2",
  fallback: [],
  validate: (v): v is Reservation[] => Array.isArray(v),
});
...
export default function Page() {
  ...
  // ここは基盤の使い方を見せる画面なので、なおさら手本であるべき
  React.useEffect(() => { setRows(store.get()); }, []);

  const save = (next: Reservation[]) => {
    setRows(next);
    const saved = store.set(next);
    // **保存の失敗を握らない。** 画面には残るのに次回は消えている、を防ぐ。
    // 知らせ先は `msg`(Alert に出る)に寄せる——この画面に伝達手段を
    // 2 つ持つと、どちらに出したか分からなくなる。
    if (!saved.ok) {
      setMsg({ kind: "danger", text: "保存できませんでした(ブラウザの空き容量を確認してください)" });
    }
  };
