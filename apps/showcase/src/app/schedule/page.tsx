...
/** 予定の保存先。**デモなのでブラウザに置く**(サーバを持たない)。 */
const store = createWebStorage<unknown[]>({
  key: "demo-schedule-events-v3",
  fallback: [],
  validate: (v): v is unknown[] => Array.isArray(v),
});
const COLORS = ["var(--color-primary)", "var(--color-success)", "var(--color-warning)", "var(--color-danger)", "#7c3aed", "#0891b2"];
const CATS: CalendarCategory[] = [
  { id: "会議", label: "会議", color: "var(--color-primary)" },
  { id: "作業", label: "作業", color: "var(--color-warning)" },
  { id: "タスク", label: "タスク", color: "var(--color-success)" },
];
const base = new Date();
...
const SEED: DemoEvent[] = [
  { id: "e1", title: "朝会", start: at(0, 9, 30), end: at(0, 10, 0), color: "var(--color-primary)", category: "会議", recur: "daily" },
  { id: "e2", title: "週次定例", start: at(1, 15, 0), end: at(1, 16, 0), color: "#7c3aed", category: "会議", recur: "weekly" },
  { id: "e4", title: "リリース作業", start: at(2, 18, 0), end: at(2, 20, 0), color: "var(--color-warning)", category: "作業" },
];
...
export default function Page() {
  ...
  // 基盤の使い方を見せる画面なので、なおさら手本であるべき
  React.useEffect(() => { setEvents(reviveEvents(JSON.stringify(store.get()))); }, []);
  const persist = (next: DemoEvent[]) => { setEvents(next); store.set(next); };
      ...
      {sel && (
        <div style={{ marginTop: 16, padding: 14, border: "1px solid var(--color-border)", borderRadius: "var(--radius)", background: "var(--color-surface)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
            <span style={{ width: 12, height: 12, borderRadius: 3, background: sel.color ?? "var(--color-primary)" }} />
      ...
      {pending && (
        <div role="presentation" style={overlay} onClick={() => setPending(null)}>
          <div role="dialog" aria-modal="true" style={modal} onClick={(e) => e.stopPropagation()}>
            ...
            <Input autoFocus value={title} onChange={(e) => setTitle(e.target.value)} placeholder="予定のタイトル" onKeyDown={(e) => { if (e.key === "Enter" && !e.nativeEvent.isComposing) create(); }}
