...
/** 付箋の保存先。**デモなのでブラウザに置く**(サーバを持たない)。 */
const store = createWebStorage<Board[]>({
  key: "demo-kanban-v2",
  fallback: [],
  validate: (v): v is Board[] => Array.isArray(v),
});
  ...
  const columns: KanbanColumn[] = React.useMemo(() => boards.map((b) => {
    ...
    const cards: KanbanCard[] = shown.map((t) => {
      ...
      return {
        ...
        accent: st === "over" ? "var(--color-danger)" : st === "today" ? "var(--color-warning)" : PRIORITY[t.priority].color,
      };
    });
    return {
      id: b.id,
      title: `${b.title}（${shown.length}${b.wip > 0 ? ` / 上限${b.wip}` : ""}）${over ? " ⚠" : ""}`,
      accent: over ? "var(--color-danger)" : b.accent,
      cards,
    };
  }), [boards, q, who]);
  ...
  return (
    <main style={{ maxWidth: 1000, margin: "2.5rem auto", padding: "0 1rem" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
        ...
        <span style={{ ...chip, borderColor: overdue > 0 ? "var(--color-danger)" : undefined, color: overdue > 0 ? "var(--color-danger)" : undefined }}>期限超過 {overdue} 件</span>
        {wipOver.length > 0 && <span style={{ ...chip, borderColor: "var(--color-danger)", color: "var(--color-danger)" }}>WIP 超過: {wipOver.map((b) => b.title).join("・")}</span>}
      </div>
