...
// **署名の検証だけサーバ**（`/api/line-demo`）。HMAC を使うのでブラウザでは動かず、
// それ以前に**チャネルシークレットを画面に置いた時点で、
// 誰でも「正しい署名」を作れます**。
// 残りは文字列を読む／組み立てるだけなので画面でよく、
// **入口を経由しないサブパスから取ります**（入口は node:crypto を巻き込む）
import {
  parseLineWebhook,
  parsePostbackData,
  eventSourceId,
} from "@platform/line/webhook-parse";
import { lineRecipientType, isValidLineRecipient, type LineRecipientType } from "@platform/line/recipient";
import {
  textMessage,
  stickerMessage,
  imageMessage,
  ...
  carouselTemplate,
  flexMessage,
} from "@platform/line/messages";
import type { LineMessage } from "@platform/line/types";
...
const MESSAGE_SAMPLES: { label: string; build: () => LineMessage }[] = [
  ...
  { label: "Flex", build: () => flexMessage("経費申請 #42", { type: "bubble", body: { type: "box", layout: "vertical", contents: [{ type: "text", text: "経費申請 #42", weight: "bold", size: "lg" }, { type: "text", text: "¥12,800", size: "xxl", color: "var(--color-success)" }] } }) },
];
...
export default function Page() {
  ...
  // 署名の判定はサーバに聞く（入力が変わるたびに問い合わせる）
  const [verified, setVerified] = React.useState<boolean | null>(null);
  React.useEffect(() => {
    if (signature === "") { setVerified(null); return; }
    let alive = true;
    void (async () => {
      const res = await fetch("/api/line-demo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ body, signature, secret }),
      });
      const r = (await res.json()) as { verified?: boolean };
      // **画面が消えた後に書き込まない**（React の警告になる）
      if (alive) setVerified(r.verified ?? false);
    })();
    return () => { alive = false; };
  ...
  return (
    <main style={{ maxWidth: 900, margin: "2.5rem auto", padding: "0 1rem" }}>
      ...
      <div style={box}>
        ...
        <Textarea value={body} onChange={(e) => setBody(e.target.value)} rows={6} style={{ marginBottom: 10, fontFamily: "var(--font-mono)", fontSize: 11 }} />
