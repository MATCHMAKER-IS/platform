...
import { isSafeExternalUrl, describeUnsafeReason } from "@platform/net";
...
async function call(url: string, init: RequestInit): Promise<Response> {
  ...
  try {
    // **リダイレクトを追わない。**
    // 許可した URL から社内アドレスへ飛ばされ、SSRF の判定をすり抜ける
    return await fetch(url, { ...init, signal: ac.signal, cache: "no-store", redirect: "manual" });
  } finally {
    clearTimeout(timer);
  }
}
...
async function handlePOST(req: Request): Promise<Response> {
  ...
  // **利用者が入力した URL を確かめる(SSRF)。**
  // この口は認可を通さないので、誰でも任意のアドレスを叩かせられる。
  // `169.254.169.254` を指定されると、こちらのサーバが
  // クラウドの資格情報を取りに行く踏み台になる
  for (const [key, value] of Object.entries(values)) {
    if (!/url|endpoint|webhook|host/i.test(key)) continue;
    if (typeof value !== "string" || value.trim() === "") continue;
    const safe = isSafeExternalUrl(value.trim());
    if (!safe.ok) {
      return NextResponse.json(
        { ok: false, message: `${key}: ${describeUnsafeReason(safe.reason)}` },
        { status: 400 },
      );
    }
  }
