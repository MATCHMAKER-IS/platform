...
import { withIdempotency, createMemoryIdempotencyStore } from "@platform/http";

/** 冪等キーの記録先（**デモなのでメモリ**。本番は Redis 実装に差し替える）。 */
...
export const POST = handleRoute(async (req: Request) => {
  const parsed = validate(InquirySchema, await req.json().catch(() => ({})));
  if (!parsed.ok) throw parsed.error; // 自動で 400 + エラーボディ

  // **二重送信を防ぐ。** 送信ボタンの連打や通信の再送で
  // **同じ問い合わせが 2 件届く**——受ける側は重複か別件か分からない。
  //
  // **公開フォームなので `scope` に利用者を使えない。**
  // ここでは送信元 IP を使う——同じ人が別内容を送るときは
  // **クライアントが違う `Idempotency-Key` を付ける**前提。
  const from = req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() ?? "anonymous";
  return withIdempotency(req, { store: idempotencyStore, scope: from }, async () => {
