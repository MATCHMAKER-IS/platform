// public-api: 見本用。LINE Webhook の署名検証をサーバで行う
/**
 * LINE Webhook の**署名検証**の見本用 API。
 *
 * 【なぜ画面から呼ばないか】
 ...
 *
 * 解析（`parseLineWebhook`）は文字列を読むだけなので画面に残しています
 * （`@platform/line/webhook-parse`）。
 * **同じ Webhook の処理でも、検証と解析は置く場所が違います。**
 */
import { verifyLineSignature } from "@platform/line";
...
async function handlePOST(req: Request): Promise<Response> {
  const rl = await limiter.check(`line-demo:${clientIp(req)}`);
  ...
  const { body, signature, secret } = parsed.value;

  return Response.json({ verified: verifyLineSignature(body, signature, secret) });
}
