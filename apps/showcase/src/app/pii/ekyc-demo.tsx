...
import {
  ...
  isEkycApproved,
  type EkycStatus,
} from "@platform/ekyc/status";
// **解析は文字列を読むだけ**なので画面でよい。
// **署名の検証はサーバ**（`/api/ekyc-demo`）——HMAC を使うので
// ブラウザでは動かず、それ以前に**秘密を画面に置いた時点で
// 誰でも「正しい署名」を作れます**
import { parseEkycWebhook } from "@platform/ekyc/webhook-parse";
...
export function EkycDemo() {
  ...
  React.useEffect(() => {
    ...
    let alive = true;
    void (async () => {
      const res = await fetch("/api/ekyc-demo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ body, signature, secret, encoding }),
      });
    ...
    return () => { alive = false; };
  }, [body, signature, secret, encoding]);
  ...
  return (
    <>
      ...
      <div style={box}>
        ...
        <Textarea value={body} onChange={(e) => setBody(e.target.value)} rows={5} style={{ marginBottom: 10, fontFamily: "var(--font-mono)", fontSize: 11 }} />
...
const client = createTrustdockClient({ apiKey: showcaseEnv.TRUSTDOCK_KEY });
