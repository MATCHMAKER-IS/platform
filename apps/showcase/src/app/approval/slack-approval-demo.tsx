...
import { buildApprovalBlocks, parseInteraction } from "@platform/slack/blocks";
import { can, resolveHierarchy } from "@platform/auth/browser";
...
const mono: React.CSSProperties = { fontFamily: "var(--font-mono)", fontSize: 11.5, whiteSpace: "pre-wrap", wordBreak: "break-word" };
...
export function SlackApprovalDemo() {
  ...
  const blocks = React.useMemo(() => buildApprovalBlocks({
    ...
    fields: [
      { label: "金額", value: `${request.amount.toLocaleString("ja-JP")} 円` },
  ...
  const add = (text: string, kind: Step["kind"] = "info") =>
    setLog((l) => [{ at: new Date().toLocaleTimeString("ja-JP"), text, kind }, ...l].slice(0, 10));
...
{JSON.stringify(blocks, null, 2)}
      ...
      <div style={box}>
        ...
        <p style={{ fontSize: 12.5, color: "var(--color-muted)", lineHeight: 1.9, margin: 0 }}>
          実際に動く受信口は <code>apps/showcase/src/app/api/slack-events/route.ts</code> にあります。
