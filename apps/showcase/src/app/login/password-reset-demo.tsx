...
// **発行と照合はサーバ（`/api/password-reset-demo`）。**
// トークンのハッシュ化に `node:crypto` を使うのでブラウザでは動かず、
// それ以前に**画面で作った再設定トークンは信用できません**。
// ここから取るものはありません（型も使っていません）。
// 強度の判定は暗号まわりの担当(@platform/crypto)。auth には無い
// 強度の目安は**計算だけ**なので画面で出してよい（依存の無い `./strength` から取る）。
// ただし**弱いパスワードを拒むのはサーバ**です——画面の判定は書き換えられます
import { passwordStrength } from "@platform/crypto/strength";
...
export function PasswordResetDemo() {
  ...
  /** サーバに指示を送る（**この画面は発行も照合もしません**）。 */
  const call = async (body: unknown): Promise<Record<string, unknown>> => {
    const res = await fetch("/api/password-reset-demo", {
      method: "POST",
  ...
  const add = (t: string) => setLog((l) => [`${new Date().toLocaleTimeString("ja-JP")}  ${t}`, ...l].slice(0, 8));
  ...
  const request = async () => {
    ...
    const issued = await call({ action: "issue", userId, driftMs: drift }) as { token: string; hashHead: string };
    setMail({
      to: email,
      token: issued.token,
    ...
    setToken("");
    setDone(false);
    add(`トークンを発行（保存されるのはハッシュ ${issued.hashHead}… のみ）`);
  };

  // 2. リンクを開いて再設定
  const submit = async () => {
    const r = await call({ action: "complete", token: token.trim(), password, driftMs: drift }) as
      { ok: boolean; reason?: string; label?: string };
    if (r.ok) {
      setDone(true);
      add("再設定を確定 → 既存セッションを無効化する");
      return;
    }
    if (r.reason === "weak") {
      add(`パスワードが弱いため拒否（${r.label ?? ""}）`);
      return;
    }
    // **理由はサーバも返しません。**「期限切れ」と「存在しない」を区別して見せると、
    // 有効なトークンを探る手がかりになります
    add("照合に失敗 → 画面には「無効なリンク」とだけ出す");
    setDone(false);
  };
      ...
      <div style={box}>
        ...
        <div style={{ display: "grid", gap: 10, maxWidth: 420 }}>
          <label style={{ display: "grid", gap: 4, fontSize: 12, color: "var(--color-muted)" }}>トークン（リンクに含まれる値）
            <Input value={token} onChange={(e) => setToken(e.target.value)} placeholder="メールのリンクから" style={{ fontFamily: "var(--font-mono)" }} />
          </label>
