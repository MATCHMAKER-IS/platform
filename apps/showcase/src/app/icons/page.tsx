  ...
  return (
    <main style={{ maxWidth: 980, margin: "2.5rem auto", padding: "0 1rem" }}>
      ...
      <p style={{ fontSize: 13, color: "var(--color-muted)", lineHeight: 1.8, marginBottom: 16 }}>
        ...
        中身は lucide で、<strong>{allCount.toLocaleString("ja-JP")} 種類すべてが使えます</strong>。
      ...
      <div style={box}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <label style={{ display: "inline-flex", alignItems: "center", gap: 8, fontSize: 12, color: "var(--color-muted)" }}>
            ...
            <span style={{ width: 34, textAlign: "right", fontFamily: "var(--font-mono)" }}>{size}px</span>
          </label>
      ...
      {filtered.length === 0 ? (
        <div style={{ ...box, textAlign: "center", color: "var(--color-muted)", fontSize: 13 }}>
          この一覧には該当がありません。
          <br />
          <strong>ここに無いだけで、lucide の {allCount.toLocaleString("ja-JP")} 種類はすべて使えます。</strong>
          下のリンクから探してください。
        </div>
      ) : (
        filtered.map((c) => (
      ...
      <div style={box}>
        ...
        <ul style={{ fontSize: 13, lineHeight: 2, margin: 0, paddingLeft: "1.2em" }}>
          <li>
            ...
            — <strong>全 {allCount.toLocaleString("ja-JP")} 種類を検索できる公式サイト。</strong>ここで見つけた名前を
      ...
      <div style={box}>
        ...
        <pre style={{ fontSize: 12, fontFamily: "var(--font-mono)", background: "var(--color-bg)", padding: 12, borderRadius: "var(--radius)", margin: 0, lineHeight: 1.8, overflowX: "auto" }}>
{`// 名前で指定（Font Awesome ライク。全 ${allCount.toLocaleString("ja-JP")} 種類）
