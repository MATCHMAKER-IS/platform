...
前回 `EventTargetLike` を自前の interface で定義したが、`Window` を受け取れなかった:
```
Type error: Type 'Window' is not assignable to type 'EventTargetLike'.
Types of property 'addEventListener' are incompatible.
```

`Window.addEventListener` は**オーバーロードを持つ**ので、単純な関数型では表現できない。

**修正**: DOM の `EventTarget` をそのまま使う。
```ts
export type EventTargetLike = EventTarget;
```

### 教訓
**DOM の型は自前で書かない**。標準の型(`EventTarget` / `Element` / `Node`)を使う。
自前で書くと、オーバーロードや readonly の違いで必ず噛み合わなくなる。

---

## 【決定的】node_modules ありの typecheck で残り 5 件が見えた

`pnpm install` 後の typecheck をいただき、**正確な残りが判明**した。

| # | ファイル | 内容 | 種類 |
|---|---|---|---|
| 1 | `session/idle-timer.ts` | `Window` が `EventTargetLike` に代入できない | **修正済み**(お手元は古い版) |
| 2 | `ui/blueprint-actions.tsx` | `tone` の `"info"` が Badge に無い | 既存のバグ |
| 3-4 | `ui/data-view.tsx` | **`t` を使っているのに `const t = useT()` を消した** | **私のバグ** |
| 5 | `ui/notice-board.tsx` | `<ul>` なのに `HTMLDivElement` を指定 | 既存のバグ |

### 【私のミス】未使用と判断して消したが、使われていた
`data-view.tsx` の `const t = useT();` を「未使用」として削除したが、
**37 行目の `aria-label={t("view.toggle")}` で使われていた**。

私の検査(import 行を除いた本文での出現を数える)が、
**JSX 内の `t(...)` を拾えていなかった**。

### 修正
- `t` を関数本体に戻す
- `"info"` → `"secondary"` に対応付け(意味が近い)
...
`notice-board.tsx` が **`<div>` と `<ul>` の両方に同じ props を渡していた**:
```tsx
if (items.length === 0) {
  return <div {...props}>{emptyText}</div>;   // ❌ HTMLDivElement 用の props
}
return <ul {...props}>...</ul>;               // ❌ HTMLUListElement 用の props
```
`HTMLAttributes<T>` の `T` は 1 つしか指定できないので、**どちらかが必ず合わない**。

**修正**: 空のときも `<ul>` にする(意味的にも一貫する)。
```tsx
return (
  <ul {...props}>
    <li>{emptyText}</li>
...
### 【学び】props を渡す要素は 1 種類に揃える
`extends React.HTMLAttributes<HTMLDivElement>` と宣言したら、
**`{...props}` を渡すのは `<div>` だけ**にする。条件分岐で別の要素にすると型が合わない。
