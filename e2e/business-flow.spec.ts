 ...
 *
 * **`.first()` を付けている理由。**
 * 「実労働」「合計」のような語は、説明文・表の見出し・値の 3 か所に出る。
 * Playwright は複数当たると `strict mode violation` で落ちるが、
 * ここで見たいのは**その語が画面に出ているか**であって、どれか 1 つではない。
 * 厳密に絞るなら `getByRole` や `data-testid` だが、
 * **画面側にテスト用の印を足すのは本末転倒**(表示に関係ないものが増える)。
 *
 * ここで見るのは次の 3 つ:
...
test.describe("勤怠から給与へ", () => {
  test("打刻した内容が集計され、そのまま金額になる", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "勤怠・有給" }).first()).toBeVisible();

    // 打刻の一覧が出ている(初期データが読み込まれている)
    await expect(page.getByText("実労働").first()).toBeVisible();

    // 集計が金額に渡っている。ここが繋がっていないと給与計算が始まらない
    await expect(page.getByText("基本賃金").first()).toBeVisible();
  ...
  test("有給の残日数と、年 5 日の義務が出る", async ({ page }) => {
    ...
    await expect(page.getByText("付与の履歴").first()).toBeVisible();
  });
});
...
test.describe("申請から承認へ", () => {
  ...
  test("Slack で承認するとき、押した人の権限を確かめる", async ({ page }) => {
    ...
    await expect(page.getByText(/権限を持ちません|403/).first()).toBeVisible();
...
test.describe("マスタの登録から書き出しへ", () => {
  test("一覧・検索・CSV 出力が繋がっている", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /マスタ管理/ }).first()).toBeVisible();
  ...
  test("参照されているものは、消す前に気づける", async ({ page }) => {
    ...
    await expect(page.getByText("被参照").first()).toBeVisible();
  });
});
