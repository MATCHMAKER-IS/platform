    ...
    <main style={{ maxWidth: 900, margin: "2.5rem auto", padding: "0 1rem" }}>
      ...
      <div style={box}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 10 }}>
          ...
          <select value={lenFrom} onChange={(e) => setLenFrom(e.target.value as LengthUnit)} style={{ ...field, width: 80 }}>
            {LENGTHS.map((u) => <option key={u} value={u}>{u}</option>)}
          </select>
          <span style={{ color: "var(--color-muted)" }}>→</span>
          <select value={lenTo} onChange={(e) => setLenTo(e.target.value as LengthUnit)} style={{ ...field, width: 80 }}>
            {LENGTHS.map((u) => <option key={u} value={u}>{u}</option>)}
          </select>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 10 }}>
          ...
          <select value={areaFrom} onChange={(e) => setAreaFrom(e.target.value as AreaUnit)} style={{ ...field, width: 80 }}>
            {AREAS.map((u) => <option key={u} value={u}>{u}</option>)}
          </select>
          <span style={{ color: "var(--color-muted)" }}>→</span>
          <select value={areaTo} onChange={(e) => setAreaTo(e.target.value as AreaUnit)} style={{ ...field, width: 80 }}>
            {AREAS.map((u) => <option key={u} value={u}>{u}</option>)}
          </select>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginBottom: 10 }}>
          ...
          <select value={tempFrom} onChange={(e) => setTempFrom(e.target.value as TempUnit)} style={{ ...field, width: 80 }}>
            {TEMPS.map((u) => <option key={u} value={u}>{u}</option>)}
          </select>
          <span style={{ color: "var(--color-muted)" }}>→</span>
          <select value={tempTo} onChange={(e) => setTempTo(e.target.value as TempUnit)} style={{ ...field, width: 80 }}>
            {TEMPS.map((u) => <option key={u} value={u}>{u}</option>)}
          </select>
