...
const TABS = [{ id: "a", label: "キャッシュ", Comp: CacheDemo }, { id: "b", label: "レート制限", Comp: RateLimitDemo }] as const;
export default function Page() {
  const [tab, setTab] = React.useState<string>("a");
  const Active = (TABS.find((t) => t.id === tab) ?? TABS[0]).Comp;
  return (
    <main style={{ maxWidth: 1100, margin: "2rem auto", padding: "0 1rem" }}>
      <h1 style={{ fontSize: "1.5rem", fontWeight: 700, marginBottom: 12 }}>キャッシュ・レート制限</h1>
